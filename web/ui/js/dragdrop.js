...
var DragDrop = {
	...
	onDragEnd : function(nwPosition, sePosition, nwOffset, seOffset) {
		...
		if (this.isOutside) {
			...
			//this.parentNode.removeChild( this );
			//tempParent.parentNode.removeChild( tempParent );
			location.replace("http://localhost:8080/menumine/removeRollUp.action?target="+this.innerHTML);
			return;
		}
		...
		this.style["top"] = "0px";
		this.style["left"] = "0px";
		//alert(this.parentNode.id);//List Name
		//alert(this.innerHTML);//InnerHTML
		
		
		document.menuMineRollUp.elements['rollUpParameters.term'].value=this.innerHTML;
		document.menuMineRollUp.elements['rollUpParameters.rollUpLabel'].value=this.parentNode.id;
