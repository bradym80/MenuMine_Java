...
<webwork:form theme="'simple'" method="'POST'" action="'addRollUp.action'" name="'menuMineRollUp'">
	<webwork:hidden theme="'simple'"  name="'rollUpParameters.term'" value="''" />
...
<webwork:iterator value="rollUpTable.rollUpElements" status="mestatus">
	...
	<tr>
	<td>
		<webwork:checkbox theme="'simple'"name="'rollUpParameters.terms'"  fieldValue="statisticalTableElement.name"/>
	</td>
	<td class="<webwork:property value="#displayClass"/>"><webwork:property value="#mestatus.count"/> </td>
		...
		<td class="<webwork:property value="#displayClass"/>"><webwork:property value="statisticalTableElement.numberInSample" /></td>
		
		<td class="<webwork:property value="#displayClass"/>">
			
			 <a href="#nowhere" onclick="javascript:confirmEliminate('<webwork:url value="'addFilterEliminatorFromRollUp.action'" > <webwork:param name="'target'" value="statisticalTableElement.name" /> </webwork:url>');" >
						(Eliminate From Results)
			</a>
			<webwork:if test="rollUpElements.size() >0"> 
			<a href="#nowhere" onclick="javascript:confirmRemove('<webwork:url value="'removeRollUpByLabel.action'" > <webwork:param name="'target'" value="statisticalTableElement.name" /> </webwork:url>'	);" >
						(Remove from RollUp )
			</a>
			
			<a href="#nowhere" onclick="javascript:toggle('<webwork:property value="statisticalTableElement.name"/>Div');" >
			<img id="<webwork:property value="statisticalTableElement.name"/>Div_image"  border="0" src="img/expand.gif" alt="expand" />  Toggle Terms
			</a> 
			</webwork:if>
	...
	<tr>
		...
		<span id="<webwork:property value="statisticalTableElement.name"/>Div" style="display:none; ">
	
			<ul id="<webwork:property value="statisticalTableElement.name"/>" class="sortable boxy" style="margin-left: 1em;">
			
			<webwork:iterator value="rollUpElements" status="mestatus">
			<li><webwork:property value="term"/></li>
			<%--
	...
	<script language="JavaScript" type="text/javascript"><!--
		...
		var list = document.getElementById("<webwork:property value="statisticalTableElement.name"/>");
