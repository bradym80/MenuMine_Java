...
#backup the last working copy of menumine for one week. Another script controls weekly backups. 
echo "backup the last working copy of menumine for one week. Another script controls weekly backups."
echo "backup the last working copy of menumine for one week. Another script controls weekly backups." >> /home/mbrady/weekly_menumine_build.log
mysql -u databaseprep -pqwaszxcV < $SQL/databaseprep.sql


#create a preparatory database called menumine_databaseprep for running initialization scripts on. 
echo "backup the last working copy of menumine for one week. Another script controls weekly backups."
echo "backup the last working copy of menumine for one week. Another script controls weekly backups." >> /home/mbrady/weekly_menumine_build.log
mysql -u databaseprep -pqwaszxcV < $SQL/databaseprep2.sql


# download flavtrak from data entry machine
echo "download flavtrak from data entry machine"
echo "download flavtrak from data entry machine" >> /home/mbrady/weekly_menumine_build.log
mysqldump -u databaseprep -pqwaszxcV -h office.fsrin.com flavtrak | mysql -u databaseprep -pqwaszxcV flavtrak >> /home/mbrady/weekly_menumine_build.log


# download 07 from data entry machine
echo "download 07 from data entry machine"
echo "download 07 from data entry machine" >> /home/mbrady/weekly_menumine_build.log
mysqldump -u databaseprep -pqwaszxcV -h office.fsrin.com 07 | mysql -u databaseprep -pqwaszxcV 07 >> /home/mbrady/weekly_menumine_build.log


# Now run the make_menumine sql command!
echo "Now run the make_menumine sql command"
echo "Now run the make_menumine sql command" >> /home/mbrady/weekly_menumine_build.log
mysql -u databaseprep -pqwaszxcV menumine_databaseprep < $SQL/make_menumine_serverside.sql >> /home/mbrady/weekly_menumine_build.log


# Update the databasestructure works!
echo "Update the databasestructure"
echo "Update the databasestructure" >> /home/mbrady/weekly_menumine_build.log
java -cp $MENUMINE/WEB-INF/classes:$XP com.fsrin.menumine.produtil.UpdateDatabaseStructureCommand >> /home/mbrady/weekly_menumine_build.log


# Now the second SQL command
echo "Now the second SQL command"
echo "Now the second SQL command" >> /home/mbrady/weekly_menumine_build.log
mysql -u databaseprep -pqwaszxcV menumine_databaseprep < $SQL/make_menumine2_serverside.sql >> /home/mbrady/weekly_menumine_build.log


# and Convert emptys to nulls
echo "and Convert emptys to nulls"
echo "and Convert emptys to nulls" >> /home/mbrady/weekly_menumine_build.log
java -cp $MENUMINE/WEB-INF/classes:$XP com.fsrin.menumine.produtil.DataEmptyToNullConverterCommand >> /home/mbrady/weekly_menumine_build.log


# Construct the keywords (takes about an hour)
echo "Construct the keywords (takes about an hour)"
echo "Construct the keywords (takes about an hour)" >> /home/mbrady/weekly_menumine_build.log
java -cp $MENUMINE/WEB-INF/classes:$XP com.fsrin.menumine.produtil.KeywordBuilderCommand >> /home/mbrady/weekly_menumine_build.log


#shutdown menumine
echo "shut down menumine"
echo "shut down menumine" >> /home/mbrady/weekly_menumine_build.log
/opt/tomcat/bin/shutdown.sh
rm -rf /opt/tomcat/work/Catalina/localhost/

#make a script to copy menumine_databaseprep and menumine_admin_databaseprep to menumine and menumine_admin
echo "Now lets move it all back to the menumine and menumine_admin from the databaseprep databases"
echo "Now lets move it all back to the menumine and menumine_admin from the databaseprep databases" >> /home/mbrady/weekly_menumine_build.log
mysql -u databaseprep -pqwaszxcV < $SQL/databasemoveback.sql >> /home/mbrady/weekly_menumine_build.log

#now startup tomcat again
echo "now startup tomcat again"
echo "now startup tomcat again" >> /home/mbrady/weekly_menumine_build.log
cd /opt/tomcat/
./bin/startup.sh