//$Id: OSCacheProvider.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
/**
 * Support for OpenSymphony OSCache. This implementation assumes that
 * identifiers have well-behaved <tt>toString()</tt> methods.
 * 
...
public class OSCacheProvider implements CacheProvider {
     ...
     * OSCache {@link Config}which reads the properties file (
     * <code>oscache.properties</code>) from the classpath. If the file
     * cannot be found or loaded, an the defaults are used.
     * 
     * @param region
     * @param properties
     * @return
    ...
    public Cache buildCache(String region, Properties properties)
        ...
        int refreshPeriod = PropertiesHelper.getInt(StringHelper.qualify(
                region, OSCACHE_REFRESH_PERIOD), OSCACHE_PROPERTIES,
        ...
        Integer capacity = PropertiesHelper.getInteger(StringHelper.qualify(
                region, OSCACHE_CAPACITY), OSCACHE_PROPERTIES);
        if (capacity != null)
