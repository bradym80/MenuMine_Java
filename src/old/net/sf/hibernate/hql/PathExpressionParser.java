//$Id: PathExpressionParser.java,v 1.3 2005/08/24 04:33:25 nick Exp $
...
public class PathExpressionParser implements Parser {
    ...
    String continueFromManyToMany(Class clazz, String[] joinColumns,
            QueryTranslator q) throws QueryException {
        start(q);
        continuation = true;
        currentName = q.createNameFor(clazz);
        q.addType(currentName, clazz);
        Queryable classPersister = q.getPersister(clazz);
        join.addJoin(classPersister.getTableName(), currentName, joinColumns,
                classPersister.getIdentifierColumnNames(), joinType);
    ...
    public void token(String token, QueryTranslator q) throws QueryException {
        ...
        if (alias != null) {
            ...
            if (!ignoreInitialJoin) {
                ...
                // here (toFromFragmentString), since anything in the ON
                // condition
            ...
            if (dotcount == 0) {
                ...
                if (currentName != null) {
                    ...
                    //processCollectionProperty(token,
                    // q.getCollectionPersister(collectionRole),
                    // collectionName);
                    continuation = false;
                } else {
                ...
                if (propertyType.isComponentType()) {
                    ...
                    dereferenceCollection(
                            token,
                            ((PersistentCollectionType) propertyType).getRole(),
                            q);
    ...
    private void dereferenceEntity(String propertyName,
            EntityType propertyType, QueryTranslator q) throws QueryException {
        ...
        ClassPersister.ENTITY_ID.equals(propertyName)
                ...
                memberPersister.hasIdentifierProperty() && memberPersister
                        .getIdentifierPropertyName().equals(propertyName))) {
            // special shortcut for id properties, skip the join!
            // this must only occur at the _end_ of a path expression
            if (componentPath.length() > 0)
                componentPath.append(StringHelper.DOT);
            componentPath.append("id");
            ...
            //String[] keyColNames =
            // memberPersister.getIdentifierColumnNames();
            String[] keyColNames;
            try {
            ...
            q.addPathAliasAndJoin(path.substring(0, path.toString()
                    .lastIndexOf(StringHelper.DOT)), name, join);
    ...
    private void dereferenceCollection(String propertyName, String role,
            QueryTranslator q) throws QueryException {
        collectionRole = role;
        ...
        String tableName = collPersister.getTableName();
        addJoin(tableName, name, colNames);
    ...
    private String getPropertyPath() {
        if (currentProperty == null) {
            ...
            if (componentPath.length() > 0) {
                return new StringBuffer().append(currentProperty).append(
                        StringHelper.DOT).append(componentPath.toString())
                        .toString();
            } else {
    ...
    protected Type getPropertyType() throws QueryException {
        ...
        if (propertyType == null)
            throw new QueryException("could not resolve property type: "
                    + propertyPath);
        return propertyType;
    }
    ...
    protected String[] currentColumns() throws QueryException {
        String propertyPath = getPropertyPath();
        String[] propertyColumns = getPropertyMapping().toColumns(currentName,
                propertyPath);
        if (propertyColumns == null)
            throw new QueryException("could not resolve property columns: "
                    + propertyPath);
    ...
    public void end(QueryTranslator q) throws QueryException {
        ...
        if (propertyType != null && propertyType.isPersistentCollectionType()) {
            collectionRole = ((PersistentCollectionType) propertyType)
                    .getRole();
    ...
    private void prepareForIndex(QueryTranslator q) throws QueryException {
        ...
        QueryableCollection collPersister = q
        ...
        if (collPersister.isOneToMany()) {
            Queryable persister = (Queryable) collPersister
                    .getElementPersister();
            ojf.addJoins(persister
                    .fromJoinFragment(collectionName, true, false), persister
                    .whereJoinFragment(collectionName, true, false));
        }

        if (!continuation)
            addJoin(collPersister.getTableName(), collectionName, keyCols);
    ...
    public void setLastCollectionElementIndexValue(String value) {
        ((CollectionElement) collectionElements.getLast()).indexValue
                .append(value);
    }
    ...
    public String getWhereColumn() throws QueryException {
        if (columns.length != 1)
            throw new QueryException(
                    "path expression ends in a composite value: " + path);
        return columns[0];
    }
    ...
    public String getCollectionSubquery() throws QueryException {
        ...
        return new StringBuffer("select ").append(
                StringHelper.join(", ", currentColumns())).append(" from ")
        /*
         * .append(collectionTable) .append(' ') .append(collectionName)
         */
        .append(join.toFromFragmentString().substring(2)) // remove initial ", "
                .append(" where ").append(
                        join.toWhereFragmentString().substring(5)) // remove
                // initial "
                // and "
                .toString();
    }
    ...
    public boolean isCollectionValued() throws QueryException {
        ...
        return collectionName != null
                && !getPropertyType().isPersistentCollectionType();
    }
    ...
    public String addFromCollection(QueryTranslator q) throws QueryException {
        ...
        if (collectionElementType == null)
            throw new QueryException(
                    "must specify 'elements' for collection valued property in from clause: "
                            + path);

        if (collectionElementType.isEntityType()) {
            // an association
            QueryableCollection collectionPersister = q
                    .getCollectionPersister(collectionRole);
            Queryable entityPersister = (Queryable) collectionPersister
                    .getElementPersister();
            ...
            if (collectionPersister.isOneToMany()) {
                ...
                String[] keyColumnNames = entityPersister
                        .getIdentifierColumnNames();
                join.addJoin(entityPersister.getTableName(), elementName,
                        collectionElementColumns, keyColumnNames, joinType);
            }
            q.addFrom(elementName, clazz, join);
            currentPropertyMapping = new CollectionPropertyMapping(
                    collectionPersister);
            return elementName;
        } else {
    ...
    public void fetch(QueryTranslator q, String entityName)
            throws QueryException {
        if (isCollectionValued()) {
            q.setCollectionToFetch(getCollectionRole(), getCollectionName(),
                    getCollectionOwnerName(), entityName);
        } else {
