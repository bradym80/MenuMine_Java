//$Id: ClauseParser.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
public class ClauseParser implements Parser {
    ...
    public void token(String token, QueryTranslator q) throws QueryException {
        ...
        if (byExpected && !lcToken.equals("by")) {
            throw new QueryException("BY expected after GROUP or ORDER: "
                    + token);
        }
            ...
            if (lcToken.equals("select")) {
                ...
                if (!byExpected)
                    throw new QueryException(
                            "GROUP or ORDER expected before BY");
                child.start(q);
                byExpected = false;
            } else {
                isClauseStart = false;
        ...
        if (!isClauseStart) {
            if (cacheSelectTokens) {
                ...
                if (child == null) {
                    throw new QueryException(
                            "query must begin with SELECT or FROM: " + token);
                } else {
                    child.token(token, q);
                }
            }
