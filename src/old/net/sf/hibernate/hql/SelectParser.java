//$Id: SelectParser.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
public class SelectParser implements Parser {
    ...
    public void token(String token, QueryTranslator q) throws QueryException {
        ...
        if (afterNew) {
            ...
            if (insideNew && !aggregate && !ready) {
                //if we are inside a new Result(), but not inside a nested
                // function
                insideNew = false;
            ...
            if (!ready || !aggregate)
                throw new QueryException(token
                        + " only allowed inside aggregate function in SELECT");
            q.appendScalarSelectToken(token);
            if ("*".equals(token))
                q.addSelectScalar(Hibernate.INTEGER); //special case
        } else if (getFunction(lctoken, q) != null) {
            // the name of an SQL function
            if (!ready)
                throw new QueryException(
                        ", expected before aggregate function in SELECT: "
                                + token);
            aggregate = true;
            ...
            if (!aggregateHasArgs(lctoken, q)) {
                q
                        .addSelectScalar(aggregateType(aggregateFuncTokenList,
                                null, q));
            ...
            if (!ready)
                throw new QueryException(
                        "( expected after aggregate function in SELECT");
            ParserHelper.parse(aggregatePathExpressionParser, q.unalias(token),
                    ParserHelper.PATH_SEPARATORS, q);

            if (aggregatePathExpressionParser.isCollectionValued()) {
                q.addCollection(aggregatePathExpressionParser
                        .getCollectionName(), aggregatePathExpressionParser
                        .getCollectionRole());
            }
            q.appendScalarSelectToken(aggregatePathExpressionParser
                    .getWhereColumn());
            q.addSelectScalar(aggregateType(aggregateFuncTokenList,
                    aggregatePathExpressionParser.getWhereColumnType(), q));
            aggregatePathExpressionParser.addAssociation(q);
        } else {
            if (!ready)
                throw new QueryException(", expected in SELECT");
            ParserHelper.parse(pathExpressionParser, q.unalias(token),
                    ParserHelper.PATH_SEPARATORS, q);
    ...
    public Type aggregateType(List funcTokenList, Type type, QueryTranslator q)
    ...
    private SQLFunction getFunction(String name, QueryTranslator q) {
        return (SQLFunction) q.getFactory().getDialect().getFunctions().get(
                name);
    }
