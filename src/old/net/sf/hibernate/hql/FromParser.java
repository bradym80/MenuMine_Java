//$Id: FromParser.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
/**
 * Parses the from clause of a hibernate query, looking for tables and aliases
 * for the SQL query.
 */
    ...
    public void token(String token, QueryTranslator q) throws QueryException {
        ...
        if (lcToken.equals(StringHelper.COMMA)) {
            ...
            if (joinType == JoinFragment.FULL_JOIN
                ...
                throw new QueryException(
                        "fetch may only be used with inner join or left outer join");
            afterFetch = true;
            ...
            if (afterAs || expectingAs) {
                ...
                if (alias == null)
                    throw new QueryException("alias not specified for: "
                            + token);
                ...
                if (afterClass) {
                    ...
                    if (p == null)
                        throw new QueryException("persister not found: "
                                + token);
                    ...
                    ParserHelper.parse(peParser, q.unalias(token),
                            ParserHelper.PATH_SEPARATORS, q);
                    if (!peParser.isCollectionValued())
                        throw new QueryException(
                                "path expression did not resolve to collection: "
                                        + token);
                ...
                if (p != null) {
                    ...
                    // semi-bad thing about this: can't re-alias another
                    // alias.....
                    alias = token;
                    expectingIn = true;
                } else {
                    ...
                    //if (joinType==NONE) throw new QueryException("path
                    // expression must be preceded by full, left, right or inner
                    // join");
