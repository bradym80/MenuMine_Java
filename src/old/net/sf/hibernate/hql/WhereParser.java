//$Id: WhereParser.java,v 1.4 2005/08/24 04:33:25 nick Exp $
...
/**
 * Parses the where clause of a hibernate query and translates it to an SQL
 * where clause.
 */
...
public class WhereParser implements Parser {
    ...
    private final PathExpressionParser pathExpressionParser;
    {
        ...
        pathExpressionParser.setUseThetaStyleJoin(true); //Need this, since
        // join condition can
        // appear inside
        // parens!
    }

    private static final Set EXPRESSION_TERMINATORS = new HashSet(); //tokens
    ...
    private static final Set EXPRESSION_OPENERS = new HashSet(); //tokens that

    // open a sub
    // expression

    private static final Set BOOLEAN_OPERATORS = new HashSet(); //tokens that
    ...
    // foo.Bar.Baz = a.B.C (maps to: bar.Baz = b.C and foo.Bar = bar.id and a.B
    // = b.id)
    // foo.Bar.Baz + a.B.C (maps to: bar.Baz + b.C and foo.Bar = bar.id and a.B
    // = b.id)
    // ( foo.Bar.Baz + 1.0 ) < 2.0 (maps to: ( bar.Baz + 1.0 ) < 2.0 and foo.Bar
    // = bar.id)

    private boolean betweenSpecialCase = false; //Inside a BETWEEN ... AND ...
    ...
    // The following variables are stacks that keep information about each
    // subexpression
    // in the list of nested subexpressions we are currently processing.

    private LinkedList nots = new LinkedList(); //were an odd or even number of

    // NOTs encountered

    private LinkedList joins = new LinkedList(); //the join string built up by

    // compound paths inside this
    // expression

    private LinkedList booleanTests = new LinkedList(); //a flag indicating if

    // the subexpression is
    // known to be boolean

    private String getElementName(
            PathExpressionParser.CollectionElement element, QueryTranslator q)
        ...
        if (element.isOneToMany) {
            ...
            if (type.isEntityType()) { //ie. a many-to-many
                ...
                name = pathExpressionParser.continueFromManyToMany(clazz,
                        element.elementColumns, q);
            } else {
                throw new QueryException(
                        "illegally dereferenced collection element");
            }
        }
        return name;
    }
    ...
    public void token(String token, QueryTranslator q) throws QueryException {
        ...
        if (expectingPathContinuation) {
            boolean pathExpressionContinuesFurther = continuePathExpression(
                    token, q);
        ...
        if (!inSubselect
                && (lcToken.equals("select") || lcToken.equals("from"))) {
            inSubselect = true;
        ...
        if (inSubselect && token.equals(StringHelper.CLOSE_PAREN)) {
            ...
            if (bracketsSinceSelect == -1) {
                ...
                try {
                    ...
                    throw new QueryException(
                            "MappingException occurred compiling subquery", me);
                }
        ...
        if (inSubselect) {
            if (token.equals(StringHelper.OPEN_PAREN))
                bracketsSinceSelect++;
        ...
        if (lcToken.equals("not")) {
            nots.addLast(new Boolean(!((Boolean) nots.removeLast())
                    .booleanValue()));
            negated = !negated;
            return; //NOTE: early return
        }
    ...
    public void end(QueryTranslator q) throws QueryException {
        if (expectingPathContinuation) {
            expectingPathContinuation = false;
            PathExpressionParser.CollectionElement element = pathExpressionParser
    ...
    private void closeExpression(QueryTranslator q, String lcToken) {
        if (((Boolean) booleanTests.removeLast()).booleanValue()) { //it was a
            // boolean
            // expression
        ...
        if (!StringHelper.CLOSE_PAREN.equals(lcToken))
            appendToken(q, StringHelper.CLOSE_PAREN);
    }
    ...
    private void openExpression(QueryTranslator q, String lcToken) {
        ...
        if (!StringHelper.OPEN_PAREN.equals(lcToken))
            appendToken(q, StringHelper.OPEN_PAREN);
    }
    ...
    private void preprocess(String token, QueryTranslator q)
        ...
        if (tokens.length > 5
                && ("elements".equals(tokens[tokens.length - 1]) || "indices"
    ...
    private void doPathExpression(String token, QueryTranslator q)
        ...
        if (pathExpressionParser.isCollectionValued()) {
            ...
            q.addQuerySpace(q.getCollectionPersister(
    ...
    private void addJoin(JoinFragment ojf, QueryTranslator q) {
        ...
        fromClause.addJoins(ojf.toFromFragmentString(),
    ...
    private void doToken(String token, QueryTranslator q) throws QueryException {
        if (q.isName(StringHelper.root(token))) { //path expression
        ...
        } else if (token.startsWith(ParserHelper.HQL_VARIABLE_PREFIX)) { //named
            // query
            // parameter
            ...
            if (persister != null) { // the name of a class
                ...
                if (InFragment.NULL == discrim
                        || InFragment.NOT_NULL == discrim) {
                    throw new QueryException(
                ...
                Object constant;
                if (token.indexOf(StringHelper.DOT) > -1
                        && (constant = ReflectHelper.getConstantValue(token)) != null) {
                    Type type;
                    try {
                        type = TypeFactory.heuristicType(constant.getClass()
                                .getName());
                    } catch (MappingException me) {
                    ...
                    if (type == null)
                        throw new QueryException(
                                "Could not determine type of: " + token);
                    try {
                        appendToken(q, ((LiteralType) type)
                                .objectToSQLString(constant));
                    } catch (Exception e) {
                        throw new QueryException(
                                "Could not format constant value to SQL literal: "
                                        + token, e);
                    }
                } else { //anything else

                    String negatedToken = negated ? (String) NEGATIONS
                            .get(token.toLowerCase()) : null;
                    if (negatedToken != null
                            && (!betweenSpecialCase || !"or"
                                    .equals(negatedToken))) {
    ...
    private void addToCurrentJoin(PathExpressionParser.CollectionElement ce) {
        addToCurrentJoin(ce.joinFragment.toWhereFragmentString()
                + ce.indexValue.toString());
    }
    ...
    private boolean continuePathExpression(String token, QueryTranslator q)
            throws QueryException {
        ...
        if (token.startsWith(".")) { // the path expression continues after a ]
            ...
            doPathExpression(getElementName(element, q) + token, q); // careful
            // with
            // this!
