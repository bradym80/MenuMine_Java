//$Id: Cascades.java,v 1.4 2005/08/24 04:33:25 nick Exp $
...
public final class Cascades {
    ...
    public abstract static class CascadingAction {
         ...
         */
        abstract void cascade(SessionImplementor session, Object child,
                Object anything) throws HibernateException;

        /**
         * Should this action be cascaded to the given (possibly uninitialized)
         * collection?
         */
        abstract Iterator getCascadableChildrenIterator(
                PersistentCollectionType collectionType, Object collection);
    ...
    public static final CascadingAction ACTION_DELETE = new CascadingAction() {
        void cascade(SessionImplementor session, Object child, Object anything)
    ...
    /**
     * A cascade point that occurs just after copying from a transient parent
    ...
    /**
     * A style of cascade that can be specified by the mapping for an
     * association. The style is specified by the <tt>cascade</tt> attribute
     * in the mapping file.
     */
    public abstract static class CascadeStyle {
         ...
         */
        /*
         * boolean doCascade(SessionImplementor session, Object child) throws
         * HibernateException { return true; };
         */
    ...
    public static final CascadeStyle STYLE_SAVE_UPDATE = new CascadeStyle() {
        boolean doCascade(CascadingAction action) {
            return action == ACTION_SAVE_UPDATE || action == ACTION_LOCK
                    || action == ACTION_REPLICATE || action == ACTION_COPY;
        }
    };
     ...
     * boolean doCascade(CascadingAction action) { return
     * action==ACTION_SAVE_UPDATE; } boolean doCascade(SessionImplementor
     * session, Object child) throws HibernateException { return
     * !session.isSaved(child); }; };
     */
    /**
     * delete
     */
    ...
    /**
     * A strategy for determining if an identifier value is an identifier of a
     * new transient instance or a previously persistent transient instance. The
     * strategy is determined by the <tt>unsaved-value</tt> attribute in the
     * mapping file.
     */
    public static class IdentifierValue {
        ...
        /**
         * Assume the transient instance is newly instantiated if its identifier
         * is null or equal to <tt>value</tt>
         */
    ...
    /**
     * Assume the transient instance is newly instantiated if the identifier is
     * null.
     */
    ...
    /**
     * A strategy for determining if a version value is an version of a new
     * transient instance or a previously persistent transient instance. The
     * strategy is determined by the <tt>unsaved-value</tt> attribute in the
     * mapping file.
     */
    public static class VersionValue {
        ...
        /**
         * Assume the transient instance is newly instantiated if its version is
         * null or equal to <tt>value</tt>
         * 
         * @param value
         *            value to compare to
         */
        ...
        public Boolean isUnsaved(Object version) throws MappingException {
            ...
            return version == null || value.equals(version) ? Boolean.TRUE
                    : Boolean.FALSE;
        }
    }

    /**
     * Assume the transient instance is newly instantiated if the version is
     * null, otherwise assume it is a detached instance.
     */
    ...
    /**
     * Assume the transient instance is newly instantiated if the version is
     * null, otherwise defer to the identifier unsaved-value.
     */
    ...
    /**
     * Assume the transient instance is newly instantiated if the version is
     * negative, otherwise assume it is a detached instance.
     */
    public static final VersionValue VERSION_NEGATIVE = new VersionValue() {
        ...
        public final Boolean isUnsaved(Object version) throws MappingException {
            ...
            if (version instanceof Number) {
                return ((Number) version).longValue() < 0l ? Boolean.TRUE
                        : Boolean.FALSE;
            } else {
     ...
     */
    private static void cascade(SessionImplementor session, Object child,
            Type type, CascadingAction action, CascadeStyle style,
            int cascadeTo, Object anything) throws HibernateException {

        if (child != null) {
            if (type.isAssociationType()) {
                if (((AssociationType) type).getForeignKeyDirection()
                        .cascadeNow(cascadeTo)) {
                    if (type.isEntityType() || type.isObjectType()) {
                        /* if ( style.doCascade(session, child) ) */action
                                .cascade(session, child, anything);
                        ...
                        if (elemType.isEntityType() || elemType.isObjectType()
                                || elemType.isComponentType()) {
                            cascadeCollection(action, style, pctype, elemType,
                                    child, cascadeVia, session, anything);
                        }
                    }
                ...
                Object[] children = componentType.getPropertyValues(child,
                        session);
                Type[] types = componentType.getSubtypes();
                for (int i = 0; i < types.length; i++) {
                    CascadeStyle componentPropertyStyle = componentType
                            .cascade(i);
                    if (componentPropertyStyle.doCascade(action))
                        cascade(session, children[i], types[i], action,
                                componentPropertyStyle, cascadeTo, anything);
                }
            }
        }
    }
    ...
    public static void cascade(SessionImplementor session,
            ClassPersister persister, Object parent,
            Cascades.CascadingAction action, int cascadeTo)
            throws HibernateException {
    ...
    public static void cascade(SessionImplementor session,
            ClassPersister persister, Object parent,
            Cascades.CascadingAction action, int cascadeTo, Object anything)
            throws HibernateException {

        if (persister.hasCascades()) { // performance opt
            if (log.isTraceEnabled())
                log.trace("processing cascades for: "
                        + persister.getClassName());
            Type[] types = persister.getPropertyTypes();
            Cascades.CascadeStyle[] cascadeStyles = persister
                    .getPropertyCascadeStyles();
            for (int i = 0; i < types.length; i++) {
                CascadeStyle style = cascadeStyles[i];
                if (style.doCascade(action)) {
                    cascade(session, persister.getPropertyValue(parent, i),
                            types[i], action, style, cascadeTo, anything);
                }
            }
            if (log.isTraceEnabled())
                log.trace("done processing cascades for: "
                        + persister.getClassName());
        }
    }
     ...
     */
    private static void cascadeCollection(CascadingAction action,
            CascadeStyle style, PersistentCollectionType collectionType,
            Type elemType, Object child, int cascadeVia,
            SessionImplementor session, Object anything)
            throws HibernateException {

        if (log.isTraceEnabled())
            log.trace("cascading to collection: " + collectionType.getRole());
        Iterator iter = action.getCascadableChildrenIterator(collectionType,
                child);
        while (iter.hasNext())
            cascade(session, iter.next(), elemType, action, style, cascadeVia,
                    anything);
     ...
     */
    private static void deleteOrphans(PersistentCollection pc,
            SessionImplementor session) throws HibernateException {
        if (pc.wasInitialized()) { //can't be any orphans if it was not
            // initialized!
    ...
    /**
     * Iterate just the elements of the collection that are already there. Don't
     * load any new elements from the database.
     */
    private static Iterator getLoadedElementsIterator(
            PersistentCollectionType collectionType, Object collection) {
    ...
    /**
     * Iterate all the collection elements, loading them from the database if
     * necessary.
     */
