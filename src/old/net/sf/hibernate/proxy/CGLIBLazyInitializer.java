//$Id: CGLIBLazyInitializer.java,v 1.2 2005/04/03 06:29:04 nick Exp $
 ...
 * library
 */
public final class CGLIBLazyInitializer extends LazyInitializer implements
        MethodInterceptor {
    ...
    public static HibernateProxy getProxy(final Class persistentClass,
        ...
        try {
            final CGLIBLazyInitializer instance = new CGLIBLazyInitializer(
                    persistentClass, interfaces, id, getIdentifierMethod,
                    setIdentifierMethod, session);
            ...
            LogFactory.getLog(LazyInitializer.class).error(
                    "CGLIB Enhancement failed", t);
            throw new HibernateException("CGLIB Enhancement failed", t);
        }
    }

    public static HibernateProxy getProxy(final Factory factory,
        ...
        final CGLIBLazyInitializer instance = new CGLIBLazyInitializer(
                persistentClass, interfaces, id, getIdentifierMethod,
                setIdentifierMethod, session);
        final HibernateProxy proxy = (HibernateProxy) factory
    ...
    public static Factory getProxyFactory(Class persistentClass,
        ...
        try {
            return (Factory) Enhancer.create(
                    (interfaces.length == 1) ? persistentClass : null,
                    interfaces, NULL_METHOD_INTERCEPTOR);
        } catch (Throwable t) {
            LogFactory.getLog(LazyInitializer.class).error(
                    "CGLIB Enhancement failed", t);
            throw new HibernateException("CGLIB Enhancement failed", t);
    ...
    private CGLIBLazyInitializer(final Class persistentClass,
            final Class[] interfaces, final Serializable id,
            final Method getIdentifierMethod, final Method setIdentifierMethod,
            final SessionImplementor session) {
        super(persistentClass, id, getIdentifierMethod, setIdentifierMethod,
                session);
    ...
    protected Object serializableProxy() {
        return new SerializableProxy(persistentClass, interfaces, id,
