//$Id: SerializableProxy.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
public final class SerializableProxy implements Serializable {
    ...
    public SerializableProxy(Class persistentClass, Class[] interfaces,
    ...
    private Object readResolve() {
        try {
            Object proxy = CGLIBLazyInitializer.getProxy(persistentClass,
                    interfaces, (getIdentifierMethodName == null) ? null
                            : getIdentifierMethodClass.getDeclaredMethod(
                                    getIdentifierMethodName, null),
                    (setIdentifierMethodName == null) ? null
                            : setIdentifierMethodClass.getDeclaredMethod(
                                    setIdentifierMethodName,
                                    setIdentifierMethodParams), id, null);
            //LazyInitializer li = HibernateProxyHelper.getLazyInitializer(
            // (HibernateProxy) proxy );
            //li.setTarget(target);
            //li.setSnapshot(snapshot);
            return proxy;
        } catch (Exception e) {
            LogFactory.getLog(CGLIBLazyInitializer.class).error(
                    "Exception deserializing proxy", e);
            throw new LazyInitializationException(
                    "could not deserialize a proxy: "
                            + MessageHelper.infoString(persistentClass, id), e);
        }
    }
