//$Id: LazyInitializer.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
public abstract class LazyInitializer {
    ...
    protected LazyInitializer(Class persistentClass, Serializable id,
    ...
    private void initializeWrapExceptions() {
        try {
            ...
            LogFactory.getLog(LazyInitializer.class).error(
                    "Exception initializing proxy", e);
        ...
        if (params == 0) {
            ...
            if ("writeReplace".equals(methodName)) {
                ...
                if (target == null && session != null)
                    target = session.getEntity(new Key(id, session.getFactory()
                            .getPersister(persistentClass)));
                if (target == null) {
                    if (replacement == null)
                 ...
                 * replacement.setSnapshot(snapshot); replacement.setTarget(
                 * (Serializable) target ); return replacement;
                 */
    ...
    /**
     * Return the underlying persistent object in the given <tt>Session</tt>,
     * or null
     */
