// $Id: Binder.java,v 1.4 2005/08/24 04:33:27 nick Exp $
...
final class Binder {
    ...
    public static void bindClass(Element node, PersistentClass model,
            Mappings mapping) throws MappingException {
        ...
        try {
            ...
            throw new MappingException("persistent class [" + className
                    + "] not found", cnfe);
        }
        ...
        boolean lazyTrue = lazyNode != null
                && "true".equals(lazyNode.getValue());
        if (proxyNode != null && (lazyNode == null || lazyTrue)) {
            try {
                model.setProxyInterface(ReflectHelper
                        .classForName(getClassName(proxyNode, mapping)));
            } catch (ClassNotFoundException cnfe) {
        ...
        model.setDiscriminatorValue((discriminatorNode == null) ? model
                .getName() : discriminatorNode.getValue());

        //DYNAMIC UPDATE
        Attribute dynamicNode = node.attribute("dynamic-update");
        model.setDynamicUpdate((dynamicNode == null) ? false : "true"
                .equals(dynamicNode.getValue()));

        //DYNAMIC UPDATE
        Attribute insertNode = node.attribute("dynamic-insert");
        model.setDynamicInsert((insertNode == null) ? false : "true"
        ...
        if (persisterNode == null) {
            ...
            try {
                model.setClassPersisterClass(ReflectHelper
                        .classForName(persisterNode.getValue()));
            } catch (ClassNotFoundException cnfe) {
    ...
    public static void bindSubclass(Element node, Subclass model,
        ...
        log.info("Mapping subclass: " + model.getName() + " -> "
    ...
    private static String getClassTableName(PersistentClass model,
            Element node, Mappings mappings) {
        Attribute tableNameNode = node.attribute("table");
        if (tableNameNode == null) {
            return mappings.getNamingStrategy().classToTableName(
                    model.getName());
        } else {
            return mappings.getNamingStrategy().tableName(
                    tableNameNode.getValue());
        }
    }

    public static void bindJoinedSubclass(Element node, Subclass model,
            Mappings mappings) throws MappingException {
        ...
        // joined subclasses
        if (model.getClassPersisterClass() == null) {
            model.getRootClass().setClassPersisterClass(
                    NormalizedEntityPersister.class);
        }
        ...
        String schema = schemaNode == null ? mappings.getSchemaName()
                : schemaNode.getValue();
        Table mytable = mappings.addTable(schema, getClassTableName(model,
                node, mappings));
        model.setTable(mytable);

        log.info("Mapping joined-subclass: " + model.getName() + " -> "
                + model.getTable().getName());
    ...
    public static void bindRootClass(Element node, RootClass model,
        ...
        String schema = schemaNode == null ? mappings.getSchemaName()
                : schemaNode.getValue();
        Table table = mappings.addTable(schema, getClassTableName(model, node,
                mappings));
        model.setTable(table);

        log.info("Mapping class: " + model.getName() + " -> "
                + model.getTable().getName());

        //MUTABLE
        Attribute mutableNode = node.attribute("mutable");
        model.setMutable((mutableNode == null)
                || mutableNode.getValue().equals("true"));
        ...
        model.setExplicitPolymorphism((polyNode != null)
        ...
        while (subnodes.hasNext()) {
            ...
            if ("id".equals(name)) {
                ...
                if (propertyName == null) {
                    bindSimpleValue(subnode, id, false,
                            RootClass.DEFAULT_IDENTIFIER_COLUMN_NAME, mappings);
                    if (id.getType() == null)
                        throw new MappingException(
                                "must specify an identifier type: "
                                        + model.getMappedClass().getName());
                    model.setIdentifierProperty(null);
                } else {
                    bindSimpleValue(subnode, id, false, propertyName, mappings);
                    id
                            .setTypeByReflection(model.getMappedClass(),
                                    propertyName);
                ...
                model.setIdentifier(id);
                if (propertyName == null) {
                    bindComponent(subnode, id, null, model.getName(), "id",
                            false, mappings);
                    model.setEmbeddedIdentifier(id.isEmbedded());
                    model.setIdentifierProperty(null);
                } else {
                    Class reflectedClass = ReflectHelper
                            .reflectedPropertyClass(model.getMappedClass(),
                                    propertyName);
                    bindComponent(subnode, id, reflectedClass, model.getName(),
                            propertyName, false, mappings);
                    Property prop = new Property(id);
                    bindProperty(subnode, prop, mappings);
                    model.setIdentifierProperty(prop);
                }
                ...
                if (val.getType() == null)
                    val.setType("version".equals(name) ? Hibernate.INTEGER
                            : Hibernate.TIMESTAMP);
                ...
                if (discrim.getType() == null) {
                    ...
                    ((Column) discrim.getColumnIterator().next())
                            .setType(Hibernate.STRING);
                }
    ...
    public static void bindColumns(final Element node, final SimpleValue model,
            final boolean isNullable, final boolean autoColumn,
            final String propertyPath, final Mappings mappings) {
        ...
        if (columnAttribute == null) {
            ...
            while (iter.hasNext()) {
                ...
                Column col = new Column(model.getType(), count++);
                bindColumn(columnElement, col, isNullable);
                col.setName(mappings.getNamingStrategy().columnName(
                        columnElement.attributeValue("name")));
                if (table != null)
                    table.addColumn(col); //table=null -> an association - fill
                // it in later
                model.addColumn(col);
                //column index
            ...
            Column col = new Column(model.getType(), 0);
            bindColumn(node, col, isNullable);
            col.setName(mappings.getNamingStrategy().columnName(
                    columnAttribute.getValue()));
            Table table = model.getTable();
            if (table != null)
                table.addColumn(col); //table=null -> an association - fill it
            // in later
            model.addColumn(col);
        }
        ...
        if (autoColumn && model.getColumnSpan() == 0) {
            Column col = new Column(model.getType(), 0);
            bindColumn(node, col, isNullable);
            col.setName(mappings.getNamingStrategy().propertyToColumnName(
                    propertyPath));
    ...
    // XML
    public static void bindSimpleValue(Element node, SimpleValue model,
            boolean isNullable, String path, Mappings mappings)
    ...
    public static void bindProperty(Element node, Property model,
            Mappings mappings) throws MappingException {
        ...
        if (type == null)
            throw new MappingException(
                    "Could not determine a property type for: "
                            + model.getName());
        ...
        model.setCascade((cascadeNode == null) ? mappings.getDefaultCascade()
                : cascadeNode.getValue());
        Attribute updateNode = node.attribute("update");
        model.setUpdateable((updateNode == null) ? true : "true"
                .equals(updateNode.getValue()));
        Attribute insertNode = node.attribute("insert");
        model.setInsertable((insertNode == null) ? true : "true"
                .equals(insertNode.getValue()));
     ...
     */
    public static void bindCollection(Element node, Collection model,
            String className, String path, Mappings mappings)
            throws MappingException {
        ...
        if (persisterNode == null) {
            ...
            try {
                model.setCollectionPersisterClass(ReflectHelper
                        .classForName(persisterNode.getValue()));
            } catch (ClassNotFoundException cnfe) {
                throw new MappingException(
        ...
        if (oneToManyNode != null) {
            ...
            String tableName;
            if (tableNode != null) {
                tableName = mappings.getNamingStrategy().tableName(
                        tableNode.getValue());
            } else {
                tableName = mappings.getNamingStrategy().propertyToTableName(
                        className, path);
            }
            Attribute schemaNode = node.attribute("schema");
            String schema = schemaNode == null ? mappings.getSchemaName()
            ...
            log.info("Mapping collection: " + model.getRole() + " -> "
                    + model.getCollectionTable().getName());
        }
        //LAZINESS
        Attribute lazyNode = node.attribute("lazy");
        if (lazyNode != null) {
        ...
        if (sortedAtt == null || sortedAtt.getValue().equals("unsorted")) {
            ...
            if (!comparatorClassName.equals("natural")) {
                try {
                    model.setComparator((Comparator) ReflectHelper
                            .classForName(comparatorClassName).newInstance());
                } catch (Exception e) {
                    throw new MappingException(
        ...
        if (cascadeAtt != null
                && cascadeAtt.getValue().equals("all-delete-orphan"))
            model.setOrphanDelete(true);

        //set up second pass
        if (model instanceof List) {
            mappings.addSecondPass(new ListSecondPass(node, mappings,
                    (List) model));
        } else if (model instanceof Map) {
            mappings.addSecondPass(new MapSecondPass(node, mappings,
                    (Map) model));
        } else if (model instanceof IdentifierCollection) {
            mappings.addSecondPass(new IdentifierCollectionSecondPass(node,
                    mappings, (IdentifierCollection) model));
        } else {
            mappings.addSecondPass(new CollectionSecondPass(node, mappings,
                    model));
        }
    }

    public static void bindManyToOne(Element node, ManyToOne model,
            String path, boolean isNullable, Mappings mappings)
            throws MappingException {
        bindColumns(node, model, isNullable, true, path, mappings);
        initOuterJoinFetchSetting(node, model);
        ...
        if (classNode != null) {
            try {
                model.setType(TypeFactory.manyToOne(ReflectHelper
                        .classForName(getClassName(classNode, mappings)), model
                        .getReferencedPropertyName()));
            } catch (Exception e) {
                throw new MappingException("Could not find class: "
                        + classNode.getValue());
            }
        }
    ...
    public static void bindAny(Element node, Any model, boolean isNullable,
            Mappings mappings) throws MappingException {
        ...
        if (metaAttribute != null) {
            ...
            while (iter.hasNext()) {
                ...
                try {
                    Object value = ((DiscriminatorType) model.getMetaType())
                            .fromString(metaValue.attributeValue("value"));
                    Class clazz = ReflectHelper.classForName(getClassName(
                            metaValue.attribute("class"), mappings));
                    values.put(value, clazz);
                } catch (ClassCastException cce) {
                    throw new MappingException(
                            "meta-type was not a DiscriminatorType: "
                                    + metaType.getName());
                } catch (HibernateException he) {
                    throw new MappingException(
                            "could not interpret meta-value", he);
                } catch (ClassNotFoundException cnfe) {
                    throw new MappingException("meta-value class not found",
                            cnfe);
                }
            }
            if (values.size() > 0)
    ...
    public static void bindOneToOne(Element node, OneToOne model,
        ...
        model
                .setForeignKeyType(constrained ? ForeignKeyDirection.FOREIGN_KEY_FROM_PARENT
        ...
        if (classNode != null) {
            try {
                model.setType(TypeFactory
                        .oneToOne(ReflectHelper.classForName(getClassName(
                                classNode, mappings)), model
                                .getForeignKeyType(), model
                                .getReferencedPropertyName()));
            } catch (Exception e) {
                throw new MappingException("Could not find class: "
                        + classNode.getValue());
    ...
    public static void bindOneToMany(Element node, OneToMany model,
            Mappings mappings) throws MappingException {
        try {
            model.setType((EntityType) Hibernate.entity(ReflectHelper
                    .classForName(getClassName(node.attribute("class"),
                            mappings))));
        } catch (ClassNotFoundException cnfe) {
    ...
    public static void bindColumn(Element node, Column model, boolean isNullable) {
        ...
        model.setNullable((nullNode != null) ? !StringHelper
                .booleanValue(nullNode.getValue()) : isNullable);
        Attribute unqNode = node.attribute("unique");
        model.setUnique(unqNode != null
                && StringHelper.booleanValue(unqNode.getValue()));
        //Attribute qtNode = node.attribute("quote");
        //model.setQuoted( qtNode!=null && StringHelper.booleanValue(
        // qtNode.getValue() ) );
     ...
     */
    public static void bindArray(Element node, Array model, String prefix,
        ...
        if (att != null) {
            try {
                model.setElementClass(ReflectHelper.classForName(getClassName(
                        att, mappings)));
            } catch (ClassNotFoundException cnfe) {
                throw new MappingException(cnfe);
            }
            ...
            while (iter.hasNext()) {
                ...
                if ("element".equals(name)) {
                    ...
                    model
                            .setElementClass(model.isPrimitiveArray() ? ((PrimitiveType) type)
                                    .getPrimitiveClass()
                                    : type.getReturnedClass());
                } else if ("one-to-many".equals(name)
                        || "many-to-many".equals(name)
                        || "composite-element".equals(name)) {
                    try {
                        model.setElementClass(ReflectHelper
                                .classForName(getClassName(subnode
    ...
    public static void bindComponent(Element node, Component model,
            Class reflectedClass, String className, String path,
            boolean isNullable, Mappings mappings) throws MappingException {
        ...
        if ("dynamic-component".equals(node.getName())) {
            ...
            try {
                model.setComponentClass(ReflectHelper
                        .classForName(getClassName(classNode, mappings)));
            } catch (ClassNotFoundException cnfe) {
        ...
        while (iter.hasNext()) {
            ...
            CollectionType collectType = CollectionType
                    .collectionTypeFromString(name);
            Value value = null;
            if (collectType != null) {
                Collection collection = collectType.create(subnode, className,
                        subpath, model.getOwner(), mappings);
                mappings.addCollection(collection);
                value = collection;
            } else if ("many-to-one".equals(name)
                    || "key-many-to-one".equals(name)) {
                value = new ManyToOne(model.getTable());
                bindManyToOne(subnode, (ManyToOne) value, subpath, isNullable,
                        mappings);
            } else if ("one-to-one".equals(name)) {
                value = new OneToOne(model.getTable(), model.getOwner()
                        .getIdentifier());
                ...
                bindSimpleValue(subnode, (SimpleValue) value, isNullable,
                        subpath, mappings);
            } else if ("component".equals(name)
                    || "dynamic-component".equals(name)
                    || "nested-composite-element".equals(name)) {
                Class subreflectedClass = (model.getComponentClass() == null) ? null
                        : ReflectHelper.reflectedPropertyClass(model
                                .getComponentClass(), propertyName);
                value = (model.getOwner() != null) ? new Component(model
                        .getOwner()) : // a class component
                        new Component(model.getTable()); // a composite element
                bindComponent(subnode, (Component) value, subreflectedClass,
                        className, subpath, isNullable, mappings);
            } else if ("parent".equals(name)) {
                model.setParentProperty(propertyName);
            }

            if (value != null)
                model.addProperty(createProperty(value, propertyName, model
                        .getComponentClass(), subnode, mappings));
        }
        ...
        while (iter.hasNext()) {
            ...
            if (prop.isFormula()) {
                throw new MappingException(
                        "properties of components may not be formulas: "
                                + prop.getName());
            }
        ...
        if (model.isDynamic()) {
            componentType = new DynamicComponentType(names, types, joinedFetch,
                    cascade);
        } else {
            ...
            componentType = new ComponentType(model.getComponentClass(), names,
                    getters, setters, foundCustomAccessor, types, joinedFetch,
                    cascade, model.getParentProperty());
        }
    ...
    private static Type getTypeFromXML(Element node) throws MappingException {
        ...
        if (typeNode == null) {
            ...
            if (type == null)
                throw new MappingException("Could not interpret type: "
                        + typeNode.getValue());
        }
        return type;
    }
    ...
    private static void initOuterJoinFetchSetting(Element node, Fetchable model) {
        ...
        if (jfNode == null) {
            ...
            if ("auto".equals(eoj)) {
                ...
                model
                        .setOuterJoinFetchSetting("true".equals(eoj) ? OuterJoinLoader.EAGER
                                : OuterJoinLoader.LAZY);
            }
        }
    }
    ...
    private static void makeIdentifier(Element node, SimpleValue model,
        ...
        if (subnode != null) {
            model.setIdentifierGeneratorStrategy(subnode
                    .attributeValue("class"));

            Properties params = new Properties();

            if (mappings.getSchemaName() != null) {
                params.setProperty(PersistentIdentifierGenerator.SCHEMA,
                        mappings.getSchemaName());
            }
            params.setProperty(PersistentIdentifierGenerator.TABLE, model
                    .getTable().getName());
            ...
            while (iter.hasNext()) {
                ...
                params.setProperty(childNode.attributeValue("name"), childNode
                        .getText());
            }
    ...
    protected static void propertiesFromXML(Element node,
            PersistentClass model, Mappings mappings) throws MappingException {
        ...
        while (iter.hasNext()) {
            ...
            Value value = null;
            if (collectType != null) {
                Collection collection = collectType.create(subnode, model
                        .getName(), propertyName, model, mappings);
                mappings.addCollection(collection);
                value = collection;
            } else if ("many-to-one".equals(name)) {
                ...
                bindSimpleValue(subnode, (SimpleValue) value, true,
                        propertyName, mappings);
            } else if ("component".equals(name)
                    || "dynamic-component".equals(name)) {
                Class reflectedClass = ReflectHelper.reflectedPropertyClass(
                        model.getMappedClass(), propertyName);
                value = new Component(model);
                bindComponent(subnode, (Component) value, reflectedClass, model
                        .getName(), propertyName, true, mappings);
            ...
            if (value != null)
                model.addNewProperty(createProperty(value, propertyName, model
                        .getMappedClass(), subnode, mappings));
        }
    }

    private static Property createProperty(Value value, String propertyName,
            Class parentClass, Element subnode, Mappings mappings)
            throws MappingException {

        if (parentClass != null && value.isSimpleValue())
            ((SimpleValue) value)
                    .setTypeByReflection(parentClass, propertyName);

        if (value instanceof ToOne) { //this is done here 'cos we might only
            // know the type here (ugly!)
            String propertyRef = ((ToOne) value).getReferencedPropertyName();
            if (propertyRef != null)
                mappings.addUniquePropertyReference(((EntityType) value
                        .getType()).getAssociatedClass(), propertyRef);
        }
    ...
    private static void handleJoinedSubclass(PersistentClass model,
            Mappings mappings, Element subnode) throws MappingException {
    ...
    private static void handleSubclass(PersistentClass model,
            Mappings mappings, Element subnode) throws MappingException {
        Subclass subclass = new Subclass(model);
     ...
     */
    public static void bindListSecondPass(Element node, List model,
            java.util.Map classes, Mappings mappings) throws MappingException {
        ...
        bindSimpleValue(subnode, iv, model.isOneToMany(),
                IndexedCollection.DEFAULT_INDEX_COLUMN_NAME, mappings);
    ...
    public static void bindIdentifierCollectionSecondPass(Element node,
            IdentifierCollection model, java.util.Map persistentClasses,
            Mappings mappings) throws MappingException {
        ...
        bindSimpleValue(subnode, id, false,
                IdentifierCollection.DEFAULT_IDENTIFIER_COLUMN_NAME, mappings);
        model.setIdentifier(id);
        makeIdentifier(subnode, id, mappings);
     ...
     * Called for Maps
     */
    public static void bindMapSecondPass(Element node, Map model,
            java.util.Map classes, Mappings mappings) throws MappingException {
        ...
        while (iter.hasNext()) {
            ...
            if ("index".equals(name)) {
                ...
                bindSimpleValue(subnode, value, model.isOneToMany(),
                        IndexedCollection.DEFAULT_INDEX_COLUMN_NAME, mappings);
                ...
                bindManyToOne(subnode, mto,
                        IndexedCollection.DEFAULT_INDEX_COLUMN_NAME, model
                                .isOneToMany(), mappings);
                model.setIndex(mto);

            } else if ("composite-index".equals(name)) {
                Component component = new Component(model.getCollectionTable());
                bindComponent(subnode, component, null, model.getRole(),
                        "index", model.isOneToMany(), mappings);
     ...
     * Called for all collections
     */
    public static void bindCollectionSecondPass(Element node, Collection model,
        ...
        if (model.isOneToMany()) {
            ...
            PersistentClass persistentClass = (PersistentClass) persistentClasses
                    .get(assocClass);
        ...
        while (iter.hasNext()) {
            ...
            if ("key".equals(name)) {
                ...
                bindSimpleValue(subnode, key, model.isOneToMany(),
                        Collection.DEFAULT_KEY_COLUMN_NAME, mappings);
                ...
                bindSimpleValue(subnode, elt, true,
                        Collection.DEFAULT_ELEMENT_COLUMN_NAME, mappings);
            } else if ("many-to-many".equals(name)) {
                ManyToOne element = new ManyToOne(model.getCollectionTable());
                model.setElement(element);
                bindManyToOne(subnode, element,
                        Collection.DEFAULT_ELEMENT_COLUMN_NAME, false, mappings);
            } else if ("composite-element".equals(name)) {
                Component element = new Component(model.getCollectionTable());
                model.setElement(element);
                bindComponent(subnode, element, null, model.getRole(),
                        "element", true, mappings);
    ...
    public static void bindRoot(Document doc, Mappings model)
            throws MappingException {

        Element hmNode = doc.getRootElement();
        Attribute schemaNode = hmNode.attribute("schema");
        model
                .setSchemaName((schemaNode == null) ? null : schemaNode
                        .getValue());
        ...
        model.setAutoImport((aiNode == null) ? true : "true".equals(aiNode
                .getValue()));
        ...
        Iterator joinedsubclassnodes = hmNode
                .elementIterator("joined-subclass");
        ...
        while (nodes.hasNext()) {
            ...
            while (returns.hasNext()) {
                ...
                Class clazz;
                try {
                    clazz = ReflectHelper.classForName(getClassName(returnElem
                            .attribute("class"), model));
                } catch (ClassNotFoundException cnfe) {
                    throw new MappingException("class not found for alias: "
                            + alias, cnfe);
                }
            ...
            while (tables.hasNext()) {
                namedQuery.addSynchronizedTable(((Element) tables.next())
                        .attributeValue("table"));
            }

            log.debug("Named sql query: " + queryName + " -> "
        ...
        while (nodes.hasNext()) {
            ...
            String rename = (renameNode == null) ? StringHelper
                    .unqualify(className) : renameNode.getValue();
            log.debug("Import: " + rename + " -> " + className);
            model.addImport(className, rename);
        }
    }

    private static PersistentClass getSuperclass(Mappings model, Element subnode)
            throws MappingException {
        ...
        try {
            ...
            throw new MappingException("extends class " + extendsValue
                    + " not found.", e);
        }
        PersistentClass superModel = model.getClass(superclass);

        if (superModel == null) {
            throw new MappingException("Cannot extend unmapped class "
                    + superclass.getName());
        }
        return superModel;
    }
    ...
    abstract static class SecondPass {
        ...
        final void doSecondPass(java.util.Map persistentClasses)
                throws MappingException {
            if (log.isDebugEnabled())
                log
                        .debug("Second pass for collection: "
                                + collection.getRole());
            ...
            if (log.isDebugEnabled()) {
                ...
                if (collection.isIndexed())
                    msg += ", index: "
                            + columns(((IndexedCollection) collection)
                                    .getIndex());
        ...
        abstract void secondPass(java.util.Map persistentClasses)
                throws MappingException;
    }

    static class CollectionSecondPass extends SecondPass {
        CollectionSecondPass(Element node, Mappings mappings,
                Collection collection) {
            super(node, mappings, collection);
        }

        void secondPass(java.util.Map persistentClasses)
                throws MappingException {
            Binder.bindCollectionSecondPass(node, collection,
                    persistentClasses, mappings);
        }
    ...
    static class IdentifierCollectionSecondPass extends SecondPass {
        ...
        void secondPass(java.util.Map persistentClasses)
                throws MappingException {
            Binder.bindIdentifierCollectionSecondPass(node,
                    (IdentifierCollection) collection, persistentClasses,
                    mappings);
        }
    ...
    static class MapSecondPass extends SecondPass {
        ...
        void secondPass(java.util.Map persistentClasses)
                throws MappingException {
            Binder.bindMapSecondPass(node, (Map) collection, persistentClasses,
                    mappings);
        }
    ...
    static class ListSecondPass extends SecondPass {
        ...
        void secondPass(java.util.Map persistentClasses)
                throws MappingException {
            Binder.bindListSecondPass(node, (List) collection,
                    persistentClasses, mappings);
        }
    ...
    abstract static class CollectionType {
        ...
        public abstract Collection create(Element node, String className,
                String path, PersistentClass owner, Mappings mappings)
                throws MappingException;
        ...
        private static final CollectionType MAP = new CollectionType("map") {
            public Collection create(Element node, String prefix, String path,
                    PersistentClass owner, Mappings mappings)
                    throws MappingException {
        ...
        private static final CollectionType PRIMITIVE_ARRAY = new CollectionType(
                "primitive-array") {
            public Collection create(Element node, String prefix, String path,
                    PersistentClass owner, Mappings mappings)
                    throws MappingException {
    ...
    private static int getOptimisticLockMode(Attribute olAtt)
            throws MappingException {
        ...
        if (olMode == null || "version".equals(olMode)) {
            ...
            throw new MappingException("Unsupported optimistic-lock style: "
                    + olMode);
        }
    }
