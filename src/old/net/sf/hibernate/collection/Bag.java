//$Id: Bag.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
/**
 * An unordered, unkeyed collection that can contain the same element multiple
 * times. The Java collections API, bizarrely, has no <tt>Bag</tt>. Most
 * developers seem to use <tt>List</tt> s to represent bag semantics, so
 * Hibernate follows this practice.
 * 
 * @author Gavin King
 */
public class Bag extends ODMGCollection implements DBag, java.util.List {
    ...
    public boolean equalsSnapshot(Type elementType) throws HibernateException {
        ...
        while (iter.hasNext()) {
            ...
            if (countOccurrences(elt, bag, elementType) != countOccurrences(
                    elt, sn, elementType))
                return false;
        }
        return true;
    ...
    private int countOccurrences(Object element, java.util.List list,
            Type elementType) throws HibernateException {
        Iterator iter = list.iterator();
        int result = 0;
        while (iter.hasNext()) {
            if (elementType.equals(element, iter.next()))
                result++;
        }
        return result;
    }
    ...
    public Serializable disassemble(CollectionPersister persister)
        ...
        for (int i = 0; i < length; i++) {
            result[i] = persister.getElementType().disassemble(bag.get(i),
                    getSession());
        }
        return result;
    }

    public Bag(SessionImplementor session, CollectionPersister persister,
            Serializable disassembled, Object owner) throws HibernateException {
        super(session);
        beforeInitialize(persister);
        Serializable[] array = (Serializable[]) disassembled;
        for (int i = 0; i < array.length; i++) {
            bag.add(persister.getElementType().assemble(array[i], session,
                    owner));
        }
        setInitialized(true);
    }
    ...
    public Iterator getDeletes(Type elemType) throws HibernateException {
        //if ( !persister.isOneToMany() ) throw new AssertionFailure("Not
        // implemented for Bags");
    ...
    /*
     * public boolean equals(Object other) { read(); return bag.equals(other); }
     * 
     * public int hashCode(Object other) { read(); return bag.hashCode(); }
     */
