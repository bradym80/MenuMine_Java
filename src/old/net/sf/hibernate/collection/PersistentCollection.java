//$Id: PersistentCollection.java,v 1.3 2005/08/24 04:33:26 nick Exp $
...
/**
 * Persistent collections are treated as value objects by Hibernate. ie. they
 * have no independent existence beyond the object holding a reference to them.
 * Unlike instances of entity classes, they are automatically deleted when
 * unreferenced and automatically become persistent when held by a persistent
 * object. Collections can be passed between different objects (change "roles")
 * and this might cause their elements to move from one database table to
 * another. <br>
 * <br>
 * Hibernate "wraps" a java collection in an instance of PersistentCollection.
 * This mechanism is designed to support tracking of changes to the collection's
 * persistent state and lazy instantiation of collection elements. The downside
 * is that only certain abstract collection types are supported and any extra
 * semantics are lost <br>
 * <br>
 * Applications should <em>never</em> use classes in this package directly,
 * unless extending the "framework" here. <br>
 * <br>
 * Changes to <em>structure</em> of the collection are recorded by the
 * collection calling back to the session. Changes to mutable elements (ie.
 * composite elements) are discovered by cloning their state when the collection
 * is initialized and comparing at flush time.
 * 
 * @author Gavin King
 */
...
public abstract class PersistentCollection implements Serializable {
    ...
    protected final boolean queueAdd(Object element) {
        if (isQueueAdditionEnabled()) {
            ...
            session.dirty(this); //needed so that we remove this collection
            // from the JCS cache
            return true;
        } else {
            return false;
    ...
    /**
     * After reading all existing elements from the database, add the queued
     * elements to the underlying collection.
     */
    public void delayedAddAll(Collection coll) {
        throw new AssertionFailure(
                "Collection does not support delayed initialization");
    }

    /**
     * After flushing, clear any "queued" additions, since the database state is
     * now synchronized with the memory state.
     */
    ...
    /**
     * Not called by Hibernate, but used by non-JDK serialization, eg. SOAP
     * libraries.
     */
    ...
    /**
     * Return the collection. Overridden on ArrayHolder!
     */
    ...
    public Object getCachedValue() {
        initialized = true; //TODO: only needed for query FETCH, so should move
        // out of here
        return this;
    }
    ...
    /**
     * Initialize the collection, if possible, wrapping any exceptions in a
     * runtime exception
     * 
     * @param writing
     *            currently obsolete
    ...
    /**
     * Could the application possibly have a direct reference to the underlying
     ...
     *             if the collection was already associated with another open
     *             session
     */
     ...
     */
    public abstract Object readFrom(ResultSet rs, CollectionPersister role,
    ...
    public abstract void writeTo(PreparedStatement st,
            CollectionPersister role, Object entry, int i, boolean writeOrder)
    ...
    /**
     * Called before any elements are read into the collection, allowing
     * appropriate initializations to occur.
     */
    ...
    /**
     * Return a new snapshot of the current state of the collection, or null if
     * no persister is passed
     */
    ...
    public final void forceInitialization() throws HibernateException {
        //TODO: check that session is open!
        if (!initialized)
     ...
     */
    public abstract boolean entryExists(Object entry, int i); //note that i
    ...
    public final Iterator queuedAdditionIterator() {
        return hasQueuedAdditions() ? additions.iterator()
    ...
    /**
     * Called before inserting rows, to ensure that any surrogate keys are fully
     * generated
     */
    ...
    static void identityRemove(Collection list, Object object,
        ...
        if (object != null && session.isSaved(object)) {
            ...
            Serializable idOfCurrent = session
                    .getEntityIdentifierIfNotUnsaved(object);
            Iterator iter = list.iterator();
            while (iter.hasNext()) {
                Serializable idOfOld = session
                        .getEntityIdentifierIfNotUnsaved(iter.next());
