//$Id: ArrayHolder.java,v 1.3 2005/08/24 04:33:26 nick Exp $
...
/**
 * A persistent wrapper for an array. Lazy initialization is NOT supported. Use
 * of Hibernate arrays is not really recommended.
 * 
 * @author Gavin King
 */
public class ArrayHolder extends PersistentCollection {
    ...
    private static final Log log = LogFactory
            .getLog(PersistentCollection.class);
    ...
    protected Serializable snapshot(CollectionPersister persister)
        ...
        int length = /* (array==null) ? tempList.size() : */Array
                .getLength(array);
        Serializable result = (Serializable) Array.newInstance(persister
                .getElementClass(), length);
        for (int i = 0; i < length; i++) {
            Object elt = /* (array==null) ? tempList.get(i) : */Array.get(
                    array, i);
            try {
    ...
    public Collection getOrphans(Serializable snapshot)
        ...
        for (int i = 0; i < sn.length; i++)
            result.add(sn[i]);
        for (int i = 0; i < sn.length; i++)
            PersistentCollection.identityRemove(result, arr[i], getSession());
        return result;
    }

    public ArrayHolder(SessionImplementor session, CollectionPersister persister)
            throws HibernateException {
        super(session);
    ...
    public boolean equalsSnapshot(Type elementType) throws HibernateException {
        ...
        for (int i = 0; i < xlen; i++) {
            if (elementType.isDirty(Array.get(snapshot, i),
                    Array.get(array, i), getSession()))
                return false;
        }
        return true;
    }
    ...
    /**
     * @see PersistentCollection#writeTo(PreparedStatement, CollectionPersister,
     *      Object, int, boolean)
     */
    ...
    public Object readFrom(ResultSet rs, CollectionPersister persister,
        ...
        // relies on the fact that elements are returned sorted by index!
        // (obsolete comment, I think?)

        Object element = persister.readElement(rs, owner, getSession());
        int index = ((Integer) persister.readIndex(rs, getSession()))
                .intValue();
    ...
    //Arrays have to do things a bit differently.

    public Object getInitialValue() throws HibernateException {
        getSession().initializeCollection(this, false);
        getSession().addArrayHolder(this);
        return array;
    }

    public void beforeInitialize(CollectionPersister persister) {
        //if (tempList==null) throw new UnsupportedOperationException("Can't
        // lazily initialize arrays");
    }
    ...
    public ArrayHolder(SessionImplementor session,
        ...
        for (int i = 0; i < cached.length; i++) {
            Array.set(array, i, persister.getElementType().assemble(cached[i],
                    session, owner));
        }
        setInitialized(true);
    }
    ...
    public Serializable disassemble(CollectionPersister persister)
        ...
        for (int i = 0; i < length; i++) {
            result[i] = persister.getElementType().disassemble(
                    Array.get(array, i), getSession());
        }

        /*
         * int length = tempList.size(); Serializable[] result = new
         * Serializable[length]; for ( int i=0; i <length; i++ ) { result[i] =
         * persister.getElementType().disassemble( tempList.get(i), session ); }
         */
    ...
    public boolean needsInserting(Object entry, int i, Type elemType)
            throws HibernateException {
        Serializable sn = getSnapshot();
        return Array.get(array, i) != null
                && (i >= Array.getLength(sn) || Array.get(sn, i) == null);
    }
    ...
    public boolean needsUpdating(Object entry, int i, Type elemType)
        ...
        return i < Array.getLength(sn)
                ...
                && elemType.isDirty(Array.get(array, i), Array.get(sn, i),
                        getSession());
    }
