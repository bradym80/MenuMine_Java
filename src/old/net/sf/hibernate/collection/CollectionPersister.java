//$Id: CollectionPersister.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
/**
 * A strategy for persisting a collection role. Defines a contract between the
 * persistence strategy and the actual persistent collection framework and
 * session. Does not define operations that are required for querying
...
public interface CollectionPersister {
     ...
     */
    public void writeKey(PreparedStatement st, Serializable key,
    ...
    /**
     * Is this collection "inverse", so state changes are not propogated to the
     * database.
     */
    ...
    /**
     * Delete the persistent state of any elements that were removed from the
     * collection
     */
    ...
    /**
     * Is this an ordered collection? (An ordered collection is ordered by the
     * initialization operation, not by sorting that happens in memory, as in
     * the case of a sorted collection.)
     */
