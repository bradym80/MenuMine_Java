//$Id: Map.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
public class Map extends PersistentCollection implements java.util.Map, DMap {
    ...
    protected Serializable snapshot(CollectionPersister persister)
        ...
        while (iter.hasNext()) {
            ...
            clonedMap.put(e.getKey(), persister.getElementType().deepCopy(
                    e.getValue()));
        }
        return clonedMap;
    }
    ...
    public Collection getOrphans(Serializable snapshot)
        ...
        PersistentCollection.identityRemoveAll(result, map.values(),
                getSession());
        return result;
    }
    ...
    public boolean equalsSnapshot(Type elementType) throws HibernateException {
        ...
        while (iter.hasNext()) {
            ...
            if (elementType.isDirty(entry.getValue(), xmap.get(entry.getKey()),
                    getSession()))
                return false;
        }
        return true;
    ...
    public void beforeInitialize(CollectionPersister persister) {
        this.map = persister.hasOrdering() ? LinkedHashCollectionHelper
                .createLinkedHashMap() : new HashMap();
    }
    ...
    public Map(SessionImplementor session, CollectionPersister persister,
        ...
        for (int i = 0; i < array.length; i += 2)
            map
                    .put(persister.getIndexType().assemble(array[i], session,
                            owner), persister.getElementType().assemble(
                            array[i + 1], session, owner));
        setInitialized(true);
    }
    ...
    public Serializable disassemble(CollectionPersister persister)
        ...
        while (iter.hasNext()) {
            java.util.Map.Entry e = (java.util.Map.Entry) iter.next();
            result[i++] = persister.getIndexType().disassemble(e.getKey(),
                    getSession());
            result[i++] = persister.getElementType().disassemble(e.getValue(),
                    getSession());
        }
        return result;
    ...
    /**
     * @see net.sf.hibernate.collection.PersistentCollection#entryExists(java.lang.Object,
     *      int)
     */
