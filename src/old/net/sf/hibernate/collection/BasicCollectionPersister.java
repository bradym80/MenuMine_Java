//$Id: BasicCollectionPersister.java,v 1.2 2005/04/03 06:29:04 nick Exp $
 ...
 * @author Gavin King
 */
public final class BasicCollectionPersister extends AbstractCollectionPersister {

    public BasicCollectionPersister(Collection collection, Configuration cfg,
    ...
    protected String generateInsertRowString() {
        Insert insert = new Insert(null).setTableName(qualifiedTableName)
                .addColumns(keyColumnNames);
        if (hasIndex)
            insert.addColumns(indexColumnNames);
        if (hasIdentifier)
            insert.addColumn(identifierColumnName);
        return insert.addColumns(elementColumnNames).toStatementString();
    }
    ...
    protected String generateUpdateRowString() {
        Update update = new Update().setTableName(qualifiedTableName)
    ...
    protected String generateDeleteRowString() {
        ...
        return new Delete().setTableName(qualifiedTableName)
                .setPrimaryKeyColumnNames(pkColumns).toStatementString();
    }
    ...
    protected int doUpdateRows(Serializable id,
        ...
        try {
            ...
            try {
                ...
                while (entries.hasNext()) {
                    ...
                    if (collection.needsUpdating(entry, i, elementType)) {
                        if (st == null)
                            st = session.getBatcher().prepareBatchStatement(
            ...
            throw new JDBCException("could not update collection rows: "
                    + MessageHelper.infoString(this, id), sqle);
        }
    }

    public String selectFragment(String alias, String suffix) {
        return selectClauseFragment(alias);
    }
    ...
    protected CollectionInitializer createCollectionInitializer(
        ...
        if (batchSize > 1) {
            ...
            Loader smallBatchLoader = new CollectionLoader(this,
                    smallBatchSize, factory);
            // the strategy for choosing batch or single load:
            return new BatchingCollectionInitializer(this, batchSize,
                    batchLoader, smallBatchSize, smallBatchLoader,
                    nonbatchLoader);
        } else {
