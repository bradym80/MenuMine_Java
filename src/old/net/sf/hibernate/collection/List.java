//$Id: List.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
/**
 * A persistent wrapper for a <tt>java.util.List</tt>. Underlying collection
 * is an <tt>ArrayList</tt>.
 * 
 * @see java.util.ArrayList
 * @author Gavin King
 */
public class List extends ODMGCollection implements java.util.List, DList,
        DArray {
    ...
    public List(SessionImplementor session, CollectionPersister persister,
            Serializable disassembled, Object owner) throws HibernateException {
        super(session);
        beforeInitialize(persister);
        Serializable[] array = (Serializable[]) disassembled;
        for (int i = 0; i < array.length; i++) {
            list.add(persister.getElementType().assemble(array[i], session,
                    owner));
        }
        setInitialized(true);
    }
    ...
    public Serializable disassemble(CollectionPersister persister)
        ...
        for (int i = 0; i < length; i++) {
            result[i] = persister.getElementType().disassemble(list.get(i),
                    getSession());
        }
        return result;
    }
