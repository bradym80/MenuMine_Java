//$Id: AbstractCollectionPersister.java,v 1.4 2005/08/24 04:33:26 nick Exp $
 ...
 * @author Gavin King
 */
public abstract class AbstractCollectionPersister implements
        CollectionMetadata, QueryableCollection {
    ...
    private static final Log log = LogFactory
            .getLog(BasicCollectionPersister.class);
    ...
    public AbstractCollectionPersister(Collection collection,
        ...
        sqlOrderByStringTemplate = hasOrder ? Template
                .renderOrderByStringTemplate(sqlOrderByString, dialect) : null;
        sqlWhereString = collection.getWhere();
        hasWhere = sqlWhereString != null;
        sqlWhereStringTemplate = hasWhere ? Template.renderWhereStringTemplate(
                sqlWhereString, dialect) : null;
        ...
        checkColumnDuplication(distinctColumns, collection.getKey()
                .getColumnIterator());
        ...
        if (!collection.isOneToMany())
            checkColumnDuplication(distinctColumns, collection.getElement()
                    .getColumnIterator());

        if (elementType.isEntityType()) {
            elementPersister = factory.getPersister(((EntityType) elementType)
                    .getAssociatedClass());
        } else {
            elementPersister = null;
        }

        qualifiedTableName = table.getQualifiedName(dialect, factory
                .getDefaultSchema());
        ...
        //unquotedElementColumnNames =
        // StringHelper.unQuote(elementColumnAliases);
        ...
        if (hasIndex = collection.isIndexed()) {
            ...
            // StringHelper.unQuote(indexColumnAliases);
            checkColumnDuplication(distinctColumns, indexedCollection
                    .getIndex().getColumnIterator());
        } else {
            indexType = null;
        ...
        if (hasIdentifier = collection.isIdentified()) {
            ...
            // StringHelper.unQuote(identifierColumnAlias);
            unquotedIdentifierColumnName = identifierColumnAlias;
            identifierGenerator = idColl.getIdentifier()
                    .createIdentifierGenerator(factory.getDialect());
            checkColumnDuplication(distinctColumns, idColl.getIdentifier()
                    .getColumnIterator());
        } else {
            identifierType = null;
        ...
        if (collection.isArray()) {
            elementClass = ((net.sf.hibernate.mapping.Array) collection)
                    .getElementClass();
        } else {
        ...
        if (elementType.isComponentType()) {
            elementPropertyMapping = new CompositeElementPropertyMapping(
                    elementColumnNames, (AbstractComponentType) elementType,
                    factory);
        } else if (!elementType.isEntityType()) {
            elementPropertyMapping = new ElementPropertyMapping(
                    elementColumnNames, elementType);
        } else {
            ClassPersister persister = factory
                    .getPersister(((EntityType) elementType)
                            .getAssociatedClass());
            if (persister instanceof PropertyMapping) { //not all
                // classpersisters
                // implement
                // PropertyMapping!
    ...
    public void initialize(Serializable key, SessionImplementor session)
        ...
        try {
            ...
            throw new JDBCException("could not initialize collection: "
                    + MessageHelper.infoString(this, key), sqle);
        }
    }

    protected abstract CollectionInitializer createCollectionInitializer(
    ...
    public String getSQLOrderByString(String alias) {
        return StringHelper.replace(sqlOrderByStringTemplate,
    ...
    public Object readElement(ResultSet rs, Object owner,
            SessionImplementor session) throws HibernateException, SQLException {
        Object element = getElementType().nullSafeGet(rs, elementColumnAliases,
                session, owner);
        return element;
    }
    ...
    public Object readIndex(ResultSet rs, SessionImplementor session)
        ...
        Object index = getIndexType().nullSafeGet(rs, indexColumnAliases,
                session, null);
        if (index == null)
            throw new HibernateException("null index column for collection: "
                    + role);
        return index;
    }

    public Object readIdentifier(ResultSet rs, SessionImplementor session)
            throws HibernateException, SQLException {
        Object id = getIdentifierType().nullSafeGet(rs,
                unquotedIdentifierColumnName, session, null);
        if (id == null)
            throw new HibernateException(
                    "null identifier column for collection: " + role);
        return id;
    }
    ...
    public void writeElement(PreparedStatement st, Object elt,
            boolean writeOrder, SessionImplementor session)
            throws HibernateException, SQLException {
        getElementType().nullSafeSet(
                st,
                elt,
                1 + (writeOrder ? 0 : keyColumnNames.length
                        + (hasIndex ? indexColumnNames.length : 0)
                        + (hasIdentifier ? 1 : 0)), session);
    }

    public void writeIndex(PreparedStatement st, Object idx,
            boolean writeOrder, SessionImplementor session)
            throws HibernateException, SQLException {
        getIndexType()
                .nullSafeSet(
                        st,
                        idx,
                        1 + keyColumnNames.length
                                + (writeOrder ? elementColumnNames.length : 0),
                        session);
    }

    public void writeIdentifier(PreparedStatement st, Object idx,
            boolean writeOrder, SessionImplementor session)
            throws HibernateException, SQLException {
        getIdentifierType().nullSafeSet(
                st,
                idx,
                1 + (writeOrder ? elementColumnNames.length
                        : keyColumnNames.length), session);
    }

    private void writeRowSelect(PreparedStatement st, Object idx,
            SessionImplementor session) throws HibernateException, SQLException {
        rowSelectType.nullSafeSet(st, idx, 1 + (hasIdentifier ? 0
                : keyColumnNames.length), session);
    }

    public void writeKey(PreparedStatement st, Serializable id,
            boolean writeOrder, SessionImplementor session)
            throws HibernateException, SQLException {
        if (id == null)
            throw new NullPointerException("null key for collection: " + role); //an
        // assertion
        getKeyType().nullSafeSet(st, id,
                1 + (writeOrder ? elementColumnNames.length : 0), session);
    }
    ...
    public String selectClauseFragment(String alias) {
        SelectFragment frag = new SelectFragment().setSuffix(
                StringHelper.EMPTY_STRING).addColumns(alias,
                elementColumnNames, elementColumnAliases).addColumns(alias,
                keyColumnNames, keyColumnAliases);
        if (hasIndex)
    ...
    /*
     * private String sqlSelectString() { //we no longer have Jon Lipsky's patch
     * to allow a Map from id's to objects SimpleSelect select = new
     * SimpleSelect() .setTableName(qualifiedTableName)
     * .addColumns(elementColumnNames); if (hasIndex)
     * select.addColumns(indexColumnNames); select.addCondition( keyColumnNames,
     * "=?" ); if (hasWhere) select.addWhereToken( " and " + sqlWhereString );
     * if (hasOrder) select.setOrderBy(sqlOrderByString); return
    ...
    public final void remove(Serializable id, SessionImplementor session)
            throws HibernateException {
        ...
        if (!isInverse) {
            ...
            try {
                PreparedStatement st = session.getBatcher()
                        .prepareBatchStatement(getSQLDeleteString());
                ...
                throw new JDBCException("could not delete collection: "
    ...
    public final void recreate(PersistentCollection collection,
        ...
        if (!isInverse) {
            ...
            try {
                ...
                throw new JDBCException("could not insert collection: "
                        + MessageHelper.infoString(this, id), sqle);
            }
        }
    }

    public final void deleteRows(PersistentCollection collection,
            Serializable id, SessionImplementor session)
            throws HibernateException {
        ...
        if (!isInverse) {
            ...
            try {
                ...
                if (entries.hasNext()) {
                    int count = 0;
                    PreparedStatement st = session.getBatcher()
                            .prepareBatchStatement(getSQLDeleteRowString());
                ...
                throw new JDBCException("could not delete collection rows: "
                        + MessageHelper.infoString(this, id), sqle);
            }
        }
    }

    public final void insertRows(PersistentCollection collection,
            Serializable id, SessionImplementor session)
            throws HibernateException {
        ...
        if (!isInverse) {
            ...
            try {
                ...
                try {
                    ...
                    if (log.isDebugEnabled())
                        log
                                .debug("done inserting rows: " + count
                                        + " inserted");
                } catch (SQLException sqle) {
                    session.getBatcher().abortBatch(sqle);
                    throw sqle;
                ...
                throw new JDBCException("could not insert collection rows: "
                        + MessageHelper.infoString(this, id), sqle);
            }
    ...
    private void checkColumnDuplication(java.util.Set distinctColumns,
            Iterator columns) throws MappingException {
        while (columns.hasNext()) {
            Column col = (Column) columns.next();
            if (!distinctColumns.add(col.getName()))
                throw new MappingException(
                        "Repeated column in mapping for collection: " + role
                                + " column: " + col.getName());
        }
    }
    ...
    public final void updateRows(PersistentCollection collection,
            Serializable id, SessionImplementor session)
            throws HibernateException {
