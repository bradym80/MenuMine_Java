//$Id: Set.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
public class Set extends ODMGCollection implements java.util.Set, DSet {
    ...
    public boolean equalsSnapshot(Type elementType) throws HibernateException {
        ...
        if (sn.size() != set.size()) {
            ...
            while (iter.hasNext()) {
                ...
                if (oldValue == null
                        || elementType.isDirty(oldValue, test, getSession()))
                    return false;
            }
            return true;
        }
    ...
    public void beforeInitialize(CollectionPersister persister) {
        this.set = persister.hasOrdering() ? LinkedHashCollectionHelper
                .createLinkedHashSet() : new HashSet();
    }
    ...
    public Set(SessionImplementor session, CollectionPersister persister,
            Serializable disassembled, Object owner) throws HibernateException {
        super(session);
        beforeInitialize(persister);
        Serializable[] array = (Serializable[]) disassembled;
        for (int i = 0; i < array.length; i++)
            set.add(persister.getElementType().assemble(array[i], session,
                    owner));
        setInitialized(true);
    }
    ...
    public Serializable disassemble(CollectionPersister persister)
        ...
        int i = 0;
        while (iter.hasNext()) {
            result[i++] = persister.getElementType().disassemble(iter.next(),
                    getSession());
        }
        return result;
    ...
    public Iterator getDeletes(Type elemType) throws HibernateException {
        ...
        while (iter.hasNext()) {
            ...
            Object oldValue = sn.get(test);
            if (oldValue != null
                    && elemType.isDirty(test, oldValue, getSession())) {
    ...
    public boolean needsInserting(Object entry, int i, Type elemType)
        ...
        // note that it might be better to iterate the snapshot but this is
        // safe,
        // assuming the user implements equals() properly, as required by the
        // Set
        // contract!
    ...
    public boolean properSubsetOf(DSet otherSet) {
        read();
        return otherSet.size() > this.set.size()
                && otherSet.containsAll(this.set);
    }
    ...
    public boolean properSupersetOf(DSet otherSet) {
        read();
        return this.set.size() > otherSet.size()
                && this.set.containsAll(otherSet);
    }
