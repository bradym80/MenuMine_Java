//$Id: ODMGCollection.java,v 1.3 2005/04/03 06:29:04 nick Exp $
 ...
 * interface and are castable to that type. However, we don't recommend that the
 * object model be "polluted" by using this interface, unless the application is
 * using the ODMG API.
 * 
 * @author Gavin King
 */
public abstract class ODMGCollection extends PersistentCollection implements
        DCollection {
    ...
    public boolean existsElement(String queryString)
        ...
        try {
            return ((Integer) getSession().filter(this,
                    "select count(*) " + queryString).iterator().next())
                    .intValue() > 0;
    ...
    public DCollection query(String queryString) throws QueryInvalidException {
        ...
        try {
            return new List(getSession(), (java.util.List) getSession().filter(
                    this, queryString));
        } catch (HibernateException he) {
            throw new QueryInvalidException(he.getMessage());
        }
