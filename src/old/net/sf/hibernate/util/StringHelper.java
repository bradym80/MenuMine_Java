// $Id: StringHelper.java,v 1.2 2005/04/03 06:29:04 nick Exp $
...
public final class StringHelper {
     ...
     * name.length()-1) : name; }
     * 
     * public static void unQuoteInPlace(String[] names) { for ( int i=0; i
     * <names.length; i++ ) names[i] = unQuote( names[i] ); }
     * 
    ...
    public static String qualify(String prefix, String name) {
        return new StringBuffer(prefix.length() + name.length() + 1).append(
                prefix).append(DOT).append(name).toString();
    }
