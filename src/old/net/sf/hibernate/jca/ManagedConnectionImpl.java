// $Id: ManagedConnectionImpl.java,v 1.2 2005/04/03 06:29:05 nick Exp $
...
public class ManagedConnectionImpl implements ManagedConnection, XAResource {
    ...
    /*
     * @todo JCA 1.0, 5.10.1 A resource adapter is required to provide support
     * for basic error logging and tracing by implementing the following
     * methods: - ManagedConnectionFactory.set/getLogWriter -
     ...
     * 
     * <p>
     * It is necessary to synchronize access to this list becase
     * <code>dissociateConnection</code> can be called asynchronously from a
     * context of a different transaction.
     * </p>
     */
     ...
     * 
     * Will throw a ResourceException if the session factory fails to create a
     * session
     */
    public Object getConnection(final Subject subject,
        ...
        log.trace("getConnection called with subject[" + subject
                + " ] connection request info[" + connectionRequestInfo + "]");
        ...
        if (log.isDebugEnabled()) {
            log.debug("Added session handle - " + active
                    + " to handles now at size " + handles.size());
        }
    ...
    private void initializeSession() throws ResourceException {
        ...
        if (session == null) {
            ...
            session = (SessionImplementor) mcf.getSessionFactory().openSession(
                    connection);
        }
    }
     ...
     * ensure proper thread context, i.e. context of original component called
     ...
     */
    private static ResourceException newResourceException(final String message,
            final Exception e) {
     ...
     * 
     * @throws ResourceException
     *             cannot close connection (with the linked exception)
     */
    ...
    /**
     * Cleans up connection handles so they can't be used again But the physical
    ...
    public void cleanup() throws ResourceException {
        ...
        try {
            ...
            final String message = "Exception closing Hibernate session "
                    + session;
        ...
        try {
            ...
            final String message = "Exception closing database connection "
                    + connection;
            re = newResourceException(message, e);
        } finally {
            connection = null;
    ...
    public void associateConnection(final Object object)
        ...
        if (!(object instanceof JCASessionImpl)) {
            throw new ResourceException(
                    "Wrong kind of connection handle to associate " + object);
        }
