//$Id: Table.java,v 1.3 2005/04/03 06:29:03 nick Exp $
...
public class Table implements RelationalModel {
    ...
    public String getQualifiedName(Dialect dialect) {
        ...
        return schema == null ? quotedName : schema
                + dialect.getSchemaSeperator() + quotedName;
    }

    public String getQualifiedName(Dialect dialect, String defaultQualifier) {
        String quotedName = getQuotedName(dialect);
        return schema == null ? (defaultQualifier == null ? quotedName
                : defaultQualifier + dialect.getSchemaSeperator() + quotedName)
    ...
    public Iterator sqlAlterStrings(Dialect dialect, Mapping p,
            TableMetadata tableInfo) throws HibernateException {

        StringBuffer root = new StringBuffer("alter table ").append(
                getQualifiedName(dialect)).append(' ').append(
        ...
        while (iter.hasNext()) {
            ...
            if (columnInfo == null) {
                ...
                StringBuffer alter = new StringBuffer(root.toString()).append(
                        ' ').append(col.getQuotedName(dialect)).append(' ')
    ...
    public String sqlCreateString(Dialect dialect, Mapping p)
        ...
        StringBuffer buf = new StringBuffer("create table ").append(
                getQualifiedName(dialect)).append(" (");

        boolean identityColumn = idValue != null
                && idValue.createIdentifierGenerator(dialect) instanceof IdentityGenerator;
        ...
        if (primaryKey != null && identityColumn) {
            pkname = ((Column) primaryKey.getColumnIterator().next())
        ...
        while (iter.hasNext()) {
            ...
            buf.append(col.getQuotedName(dialect)).append(' ').append(
                    col.getSqlType(dialect, p));
    ...
    public String sqlDropString(Dialect dialect) {
        ...
        buf.append(getQualifiedName(dialect)).append(
    ...
    public Index createIndex(String name, List indexColumns) {
        if (name == null)
            name = "IX" + uniqueColumnString(indexColumns.iterator());
    ...
    public ForeignKey createForeignKey(String keyName, List keyColumns,
        ...
        if (keyName == null)
            keyName = "FK" + uniqueColumnString(keyColumns.iterator());
        ...
        if (fk == null) {
            ...
            keyName += Integer
                    .toHexString(referencedClass.getName().hashCode())
                    .toUpperCase();
            if (fk.getReferencedClass() != referencedClass)
                fk = createForeignKey(keyName, keyColumns, referencedClass);
        }
