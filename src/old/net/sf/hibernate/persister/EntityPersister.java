//$Id: EntityPersister.java,v 1.4 2005/08/24 04:33:28 nick Exp $
 ...
 * @author Gavin King
 */
public class EntityPersister extends AbstractEntityPersister implements
        Queryable {
    ...
    public void postInstantiate() throws MappingException {
        ...
        String selectForUpdate = factory.getDialect().supportsForUpdate() ? generateSelectForUpdateString()
                : generateSelectString();

        loaders.put(LockMode.UPGRADE, new SimpleEntityLoader(this,
                selectForUpdate, LockMode.UPGRADE));

        String selectForUpdateNowait = factory.getDialect()
                .supportsForUpdateNowait() ? generateSelectForUpdateNowaitString()
                : selectForUpdate;

        loaders.put(LockMode.UPGRADE_NOWAIT, new SimpleEntityLoader(this,
                selectForUpdateNowait, LockMode.UPGRADE_NOWAIT));
    ...
    protected String generateDeleteString() {
        return new Delete().setTableName(getTableName())
    ...
    protected String generateInsertString(boolean identityInsert,
        ...
        Insert insert = new Insert(getDialect()).setTableName(getTableName());
        for (int i = 0; i < getHydrateSpan(); i++) {
            if (includeProperty[i])
                insert.addColumns(propertyColumnNames[i]);
        }
        final boolean includeDiscriminator = isPolymorphic()
                && InFragment.NULL != discriminatorSQLValue
                && InFragment.NOT_NULL != discriminatorSQLValue;
        if (includeDiscriminator)
            insert.addColumn(getDiscriminatorColumnName(),
                    discriminatorSQLValue.toString());
        if (!identityInsert) {
    ...
    protected String generateSelectString() {
        ...
        return select.addCondition(getIdentifierColumnNames(), "=?")
                .toStatementString();
    }
    ...
    protected String generateConcreteSelectString(boolean[] includeProperty) {
        SimpleSelect select = new SimpleSelect().setTableName(getTableName())
                .addColumns(getIdentifierColumnNames());
        for (int i = 0; i < getPropertyNames().length; i++) {
            if (includeProperty[i]) { //ie. not a formula, updateable
                select.addColumns(propertyColumnNames[i],
    ...
    protected String generateUpdateString(boolean[] includeProperty,
        ...
        if (optimisticLockMode() > Versioning.OPTIMISTIC_LOCK_VERSION
                && oldFields != null) {
            boolean[] includeInWhere = optimisticLockMode() == Versioning.OPTIMISTIC_LOCK_ALL ? getPropertyUpdateability()
                    : includeProperty;
            for (int i = 0; i < getHydrateSpan(); i++) {
                if (includeInWhere[i]) {
                    if (oldFields[i] == null) {
    ...
    private Update generateUpdate(boolean[] includeProperty) {
        Update update = new Update().setTableName(getTableName())
    ...
    protected String generateLockString() {
        ...
        SimpleSelect select = new SimpleSelect().setTableName(getTableName())
                .addColumn(getIdentifierColumnNames()[0]).addCondition(
    ...
    protected int dehydrate(Serializable id, Object[] fields,
            boolean[] includeProperty, PreparedStatement st,
        ...
        for (int j = 0; j < getHydrateSpan(); j++) {
            if (includeProperty[j]) {
                getPropertyTypes()[j]
                        .nullSafeSet(st, fields[j], index, session);
    ...
    public Object load(Serializable id, Object optionalObject,
        ...
        try {
            return ((UniqueEntityLoader) loaders.get(lockMode)).load(session,
                    id, optionalObject);
        } catch (SQLException sqle) {
            throw new JDBCException("could not load: "
    ...
    public Serializable insert(Object[] fields, Object object,
        ...
        if (useDynamicInsert()) {
            ...
            return insert(fields, notNull, generateInsertString(true, notNull),
                    object, session);
        } else {
    ...
    public void insert(Serializable id, Object[] fields, Object object,
        ...
        if (useDynamicInsert()) {
            boolean[] notNull = getNotNullInsertableColumns(fields);
            insert(id, fields, notNull, generateInsertString(false, notNull),
                    object, session);
        } else {
            insert(id, fields, getPropertyInsertability(),
                    getSQLInsertString(), object, session);
        }
    }
     ...
     */
    public Serializable insert(Object[] fields, boolean[] notNull, String sql,
            Object object, SessionImplementor session)
            throws HibernateException {
    ...
    public void delete(Serializable id, Object version, Object object,
        ...
        try {
            ...
            //Render the SQL query
            final PreparedStatement statement;// = session.getPreparedStatement(
            // sqlDelete() );
            if (isVersioned()) {
                statement = session.getBatcher().prepareStatement(
                        getSQLDeleteString());
            } else {
            ...
            try {
                ...
                if (isVersioned()) {
                    getVersionType().nullSafeSet(statement, version,
    ...
    public void update(Serializable id, Object[] fields, int[] dirtyFields,
        ...
        update(id, fields, oldFields, propsToUpdate, oldVersion, object,
                updateString, session);
    }

    protected void update(Serializable id, Object[] fields, Object[] oldFields,
            boolean[] includeProperty, Object oldVersion, Object object,
            String sql, SessionImplementor session) throws HibernateException {
        if (log.isTraceEnabled()) {
        ...
        try {
            ...
            try {
                ...
                int index = dehydrate(id, fields, includeProperty, statement,
                        session);

                if (isVersioned()
                        && optimisticLockMode() == Versioning.OPTIMISTIC_LOCK_VERSION) {
                    getVersionType().nullSafeSet(statement, oldVersion, index,
                            session);
                    check(statement.executeUpdate(), id);
                } else if (optimisticLockMode() > Versioning.OPTIMISTIC_LOCK_VERSION
                        && oldFields != null) {

                    boolean[] includeOldField = optimisticLockMode() == Versioning.OPTIMISTIC_LOCK_ALL ? getPropertyUpdateability()
                            : includeProperty;
                    for (int j = 0; j < getHydrateSpan(); j++) {
                        if (includeOldField[j] && oldFields[j] != null) {
                            getPropertyTypes()[j].nullSafeSet(statement,
                                    oldFields[j], index, session);
                            index += propertyColumnSpans[j];
                        }
                    }
            ...
            throw new JDBCException("could not update: "
                    + MessageHelper.infoString(this, id), sqle);
        }
    ...
    public EntityPersister(PersistentClass model,
        ...
        if (model.isPolymorphic()) {
            ...
            if (model.isDiscriminatorValueNull()) {
                ...
                try {
                    ...
                    discriminatorValue = dtype.stringToObject(model
                            .getDiscriminatorValue());
                    discriminatorSQLValue = dtype
                            .objectToSQLString(discriminatorValue);
                } catch (ClassCastException cce) {
                    throw new MappingException("Illegal discriminator type: "
                            + discriminatorType.getName());
                } catch (Exception e) {
                    throw new MappingException(
                            "Could not format discriminator value to SQL string",
                            e);
                }
                distinctColumns.add(discriminatorColumnName); //don't do this
                // check for the
                // case of
                // null/not null
                // discriminator
        ...
        subclassFormulaClosure = (String[]) formulas
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        ...
        subclassColumnAliasClosure = (String[]) aliases
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        ...
        sqlInsertString = generateInsertString(false,
                getPropertyInsertability());
        sqlIdentityInsertString = isIdentifierAssignedByInsert() ? generateInsertString(
        ...
        if (model.isPolymorphic()) {
            ...
            while (iter.hasNext()) {
                ...
                if (sc.isDiscriminatorValueNull()) {
                    subclassesByDiscriminatorValue.put(NULL_DISCRIMINATOR, sc
                            .getMappedClass());
                } else if (sc.isDiscriminatorValueNotNull()) {
                    subclassesByDiscriminatorValue.put(NOT_NULL_DISCRIMINATOR,
                            sc.getMappedClass());
                } else {
                    try {
                        DiscriminatorType dtype = (DiscriminatorType) discriminatorType;
                        subclassesByDiscriminatorValue.put(dtype
                                .stringToObject(sc.getDiscriminatorValue()), sc
                                .getMappedClass());
                    } catch (ClassCastException cce) {
                        throw new MappingException(
                                "Illegal discriminator type: "
                                        + discriminatorType.getName());
                    } catch (Exception e) {
                        throw new MappingException(
                                "Error parsing discriminator value", e);
                    }
                }
            }
        }
    ...
    public String queryWhereFragment(String name, boolean innerJoin,
            boolean includeSubclasses) throws MappingException {

        if (innerJoin && (forceDiscriminator || isInherited())) {
            InFragment frag = new InFragment().setColumn(name,
                    getDiscriminatorColumnName());
            Class[] subclasses = getSubclassClosure();
            for (int i = 0; i < subclasses.length; i++) {
                frag.addValue(((Queryable) factory.getPersister(subclasses[i]))
                        .getDiscriminatorSQLValue());
            }
            StringBuffer buf = new StringBuffer(50).append(" and ").append(
                    frag.toFragmentString());
            if (hasWhere())
                buf.append(" and ").append(getSQLWhereString(name));
            return buf.toString();
        } else {
    ...
    public String propertySelectFragment(String name, String suffix) {
        ...
        if (hasSubclasses())
            frag.addColumn(name, getDiscriminatorColumnName(),
                    getDiscriminatorAlias());
        return frag.addColumns(name, subclassColumnClosure,
                subclassColumnAliasClosure).addFormulas(name,
                subclassFormulaTemplateClosure, subclassFormulaAliasClosure)
                .toFragmentString();
    }
