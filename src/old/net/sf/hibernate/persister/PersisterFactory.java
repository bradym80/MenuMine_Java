...
/**
 * Factory for <tt>ClassPersister</tt> and <tt>CollectionPersister</tt>
 * instances
 * 
 * @author Gavin King
 */
public final class PersisterFactory {
    ...
    // TODO: is it really neceassry to provide Configuration to
    // CollectionPersisters ? Should it not be enough with associated class ?
    ...
    private static final Class[] COLLECTION_PERSISTER_CONSTRUCTOR_ARGS = new Class[] {
            PersistentClass.class, Configuration.class,
    ...
    public static CollectionPersister createCollectionPersister(
            Configuration cfg, Collection model,
            SessionFactoryImplementor factory) throws HibernateException {
        Class persisterClass = model.getCollectionPersisterClass();
        if (persisterClass == null) { // default behavior
            return model.isOneToMany() ? (CollectionPersister) new OneToManyPersister(
                    model, cfg, factory)
                    : (CollectionPersister) new BasicCollectionPersister(model,
                            cfg, factory);
        } else {
    ...
    private static ClassPersister create(Class persisterClass,
            PersistentClass model, SessionFactoryImplementor factory)
            throws HibernateException {
        Constructor pc;
        try {
            pc = persisterClass.getConstructor(PERSISTER_CONSTRUCTOR_ARGS);
        } catch (Exception e) {
            throw new MappingException("Could not get constructor for "
                    + persisterClass.getName(), e);
        }

        try {
            return (ClassPersister) pc.newInstance(new Object[] { model,
                    factory });
            ...
            if (e instanceof HibernateException) {
                ...
                throw new MappingException("Could not instantiate persister "
                        + persisterClass.getName(), e);
            }
        } catch (Exception e) {
            throw new MappingException("Could not instantiate persister "
    ...
    private static CollectionPersister create(Class persisterClass,
            Configuration cfg, Collection model,
            SessionFactoryImplementor factory) throws HibernateException {
        Constructor pc;
        try {
            pc = persisterClass
                    .getConstructor(COLLECTION_PERSISTER_CONSTRUCTOR_ARGS);
        } catch (Exception e) {
            throw new MappingException("Could not get constructor for "
                    + persisterClass.getName(), e);
        }
        ...
        try {
            return (CollectionPersister) pc.newInstance(new Object[] { model,
                    cfg, factory });
        } catch (InvocationTargetException ite) {
            Throwable e = ite.getTargetException();
            if (e instanceof HibernateException) {
