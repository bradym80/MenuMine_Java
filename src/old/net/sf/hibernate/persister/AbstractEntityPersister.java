//$Id: AbstractEntityPersister.java,v 1.6 2005/08/24 04:33:28 nick Exp $
...
/**
 * Superclass for built-in mapping strategies. Implements functionality common
 * to both mapping strategies. <br>
 * <br>
 * May be considered an immutable view of the mapping object. <br>
 * 
 * @author Gavin King
 */
public abstract class AbstractEntityPersister extends AbstractPropertyMapping
        implements OuterJoinLoadable, Queryable, ClassMetadata,
        UniqueKeyLoadable, SQLLoadable {
    ...
    private final Method proxyGetIdentifierMethod;

    private final Method proxySetIdentifierMethod;
    ...
    private final HashMap gettersByPropertyName = new HashMap();

    private final HashMap settersByPropertyName = new HashMap();

    private final HashMap typesByPropertyName = new HashMap();
    ...
    public String identifierSelectFragment(String name, String suffix) {
        return new SelectFragment().setSuffix(suffix).addColumns(name,
                getIdentifierColumnNames(), getIdentifierAliases())
                .toFragmentString().substring(2); //strip leading ", "
    }
    ...
    public void setPropertyValues(Object object, Object[] values)
        ...
        try {
            ...
            throw new PropertyAccessException(t,
                    ReflectHelper.PROPERTY_ACCESS_EXCEPTION, true, mappedClass,
    ...
    public Object[] getPropertyValues(Object object) throws HibernateException {
        try {
            ...
            throw new PropertyAccessException(t,
                    ReflectHelper.PROPERTY_ACCESS_EXCEPTION, false,
                    mappedClass, ReflectHelper.getPropertyName(t, optimizer));
        }
     ...
     */
    public int[] findDirty(Object[] x, Object[] y, Object object,
            SessionImplementor session) throws HibernateException {
        int[] props = TypeFactory.findDirty(propertyTypes, x, y,
        ...
        if (props == null) {
            ...
            if (log.isTraceEnabled()) {
                for (int i = 0; i < props.length; i++) {
                    log.trace(StringHelper.qualify(className,
                            propertyNames[props[i]])
                            + " is dirty");
                }
            }
            return props;
    ...
    public int[] findModified(Object[] old, Object[] current, Object object,
            SessionImplementor session) throws HibernateException {
        int[] props = TypeFactory.findModified(propertyTypes, old, current,
                propertyUpdateability, session);
        if (props == null) {
            return null;
        } else {
        ...
        if (hasEmbeddedIdentifier) {
            ...
            if (identifierGetter == null)
                throw new HibernateException(
                        "The class has no identifier property: " + className);
            ...
            if (optimizer != null) {
                try {
                    ...
                    throw new InstantiationException(
                            "Could not instantiate entity with CGLIB: ",
                            mappedClass, t);
                }
            } else {
                try {
                    ...
                    throw new InstantiationException(
                            "Could not instantiate entity: ", mappedClass, e);
                }
            }
    ...
    protected void initPropertyPaths(Mapping mapping) throws MappingException {
        ...
        for (int i = 0; i < propertyNames.length; i++) {
            initPropertyPaths(propertyNames[i], propertyTypes[i],
                    getActualPropertyColumnNames(i), getFormulaTemplate(i),
                    mapping);
        }

        String idProp = getIdentifierPropertyName();
        if (idProp != null)
            initPropertyPaths(idProp, getIdentifierType(),
                    getIdentifierColumnNames(), mapping);
        if (hasEmbeddedIdentifier())
            initPropertyPaths(null, getIdentifierType(),
                    getIdentifierColumnNames(), mapping);
        initPropertyPaths(ENTITY_ID, getIdentifierType(),
                getIdentifierColumnNames(), mapping);

        if (isPolymorphic()) {
            addPropertyPath(ENTITY_CLASS, getDiscriminatorType(),
    ...
    protected AbstractEntityPersister(PersistentClass model,
        ...
        sqlWhereStringTemplate = sqlWhereString == null ? null : Template
        ...
        if (optimisticLockMode > Versioning.OPTIMISTIC_LOCK_VERSION
            ...
            throw new MappingException(
                    "optimistic-lock setting requires dynamic-update=\"true\": "
                            + className);
        }
        ...
        identifierGenerator = model.getIdentifier().createIdentifierGenerator(
                dialect);
        useIdentityColumn = identifierGenerator instanceof IdentityGenerator;
        identitySelectString = useIdentityColumn ? dialect
                .getIdentitySelectString() : null;
        ...
        if (unsavedValue == null || "null".equals(unsavedValue)) {
            ...
            try {
                ...
                throw new MappingException("Bad identifier type: "
                        + idType.getClass().getName());
            } catch (Exception e) {
                throw new MappingException(
                        "Could not parse identifier unsaved-value: "
                                + unsavedValue);
            }
        }
        ...
        if (model.isVersioned()) {
            versionColumnName = ((Column) model.getVersion()
                    .getColumnIterator().next()).getQuotedName(dialect);
        } else {
        ...
        if (versionUnsavedValue == null
                || "undefined".equals(versionUnsavedValue)) {
            ...
            /*
             * used to be none and any strategies but this is kind of a non
             * sense for version especially for none since an 'update where'
             * would be generated Lot's of hack to support none ?
             */
        } else {
            // this should not happend since the DTD prevent it.
            throw new MappingException(
                    "Could not parse version unsaved-value: "
                            + versionUnsavedValue);
        }
        ...
        while (iter.hasNext()) {
            ...
            if (cascadeStyles[i] != Cascades.STYLE_NONE)
                foundCascade = true;

            i++;
        }

        optimizer = !foundCustomAccessor
                && Environment.useReflectionOptimizer() ? ReflectHelper
                .getBulkBean(mappedClass, getterNames, setterNames, types)
                : null;
        ...
        if (hasProxy) {
            ...
            if (mappedClass.isInterface())
                proxyInterfaceSet.add(mappedClass);
                    ...
                    if (subclassProxy == null)
                        throw new MappingException(
                                "All subclasses must also have proxies: "
                                        + mappedClass.getName());
                    if (!subclass.getMappedClass().equals(subclassProxy))
                        proxyInterfaceSet.add(subclassProxy);
                }
            }

            proxyInterfaces = (Class[]) proxyInterfaceSet.toArray(NO_CLASSES);

            proxyFactory = hasProxy ? CGLIBLazyInitializer.getProxyFactory(
                    mappedClass, proxyInterfaces) : null;
    ...
    protected void initSubclassPropertyAliasesMap(PersistentClass model)
        ...
        if (hasIdentifierProperty()) {
            subclassPropertyAliases.put(getIdentifierPropertyName(),
                    getIdentifierAliases());
            subclassPropertyAliases.put(ENTITY_ID, getIdentifierAliases());
        } else if (hasEmbeddedIdentifier()) {
            // Fetch embedded identifiers propertynames from the "virtual"
            ...
            for (int i = 0; i < idPropertyNames.length; i++) {
                subclassPropertyAliases.put(idColumns[i],
                        new String[] { idPropertyNames[i] });
            }
        }
    ...
    protected void initLockers() {
        ...
        String lockExclusiveString = getDialect().supportsForUpdate() ? lockString
                + " for update"
                : lockString;
        lockers.put(LockMode.UPGRADE, lockExclusiveString);
        String lockExclusiveNowaitString = getDialect()
                .supportsForUpdateNowait() ? lockString + " for update nowait"
                : lockExclusiveString;
    ...
    private boolean initHasCollections(Type[] types) {
        for (int i = 0; i < types.length; i++) {
            if (types[i].isPersistentCollectionType()) {
                ...
                if (initHasCollections(((AbstractComponentType) types[i])
                        .getSubtypes()))
                    return true;
            }
        }
    ...
    protected UniqueEntityLoader createEntityLoader(
        ...
        if (batchSize > 1) {
            ...
            Loader smallBatchLoader = new EntityLoader(this, smallBatchSize,
                    factory);
            return new BatchingEntityLoader(this, batchSize, batchLoader,
                    smallBatchSize, smallBatchLoader, nonBatchLoader);
        } else {
    ...
    protected void createUniqueKeyLoaders(SessionFactoryImplementor factory)
        ...
        for (int i = 0; i < propertyNames.length; i++) {
            String[] columns = (String[]) uniqueKeyColumns
                    .get(propertyNames[i]);
            if (columns != null) {
                ...
                if (uniqueKeyType.isEntityType()) {
                    Class clazz = ((EntityType) uniqueKeyType)
                            .getAssociatedClass();
                    uniqueKeyType = factory.getPersister(clazz)
                            .getIdentifierType();
                }
                uniqueKeyLoaders.put(propertyNames[i], new EntityLoader(this,
                        columns, uniqueKeyType, 1, factory));
            }
        }
    }
    ...
    public String[] getSubclassPropertyColumnAliases(String propertyName,
            String suffix) {
        String rawAliases[] = (String[]) subclassPropertyAliases
                .get(propertyName);
    ...
    public String[] getIdentifierAliases(String suffix) {
        // NOTE: this assumes something about how propertySelectFragment is
        // implemented by the subclass!
        // was toUnqotedAliasStrings( getIdentiferColumnNames() ) before - now
        // tried
    ...
    public String[] getPropertyAliases(String suffix, int i) {
        ...
        return new Alias(suffix)
                .toUnquotedAliasStrings(getPropertyColumnNames(i));
    }

    public String getDiscriminatorAlias(String suffix) {
        // NOTE: this assumes something about how propertySelectFragment is
        // implemented by the subclass!
        // was toUnqotedAliasStrings( getdiscriminatorColumnName() ) before -
        // now tried
        // to remove that unqoting and missing aliases..
        return hasSubclasses() ? new Alias(suffix)
    ...
    public Type getPropertyType(String propertyName) throws MappingException {
        ...
        if (propertyType == null)
            throw new MappingException("property does not exist: "
                    + propertyName);
        return propertyType;
    }
    ...
    public Object getCurrentVersion(Serializable id, SessionImplementor session)
        ...
        try {
            ...
            PreparedStatement st = session.getBatcher().prepareStatement(
            ...
            try {
                ...
                try {
                    ...
                    if (!isVersioned())
                        return this;
                    return getVersionType().nullSafeGet(rs,
            ...
            throw new JDBCException("could not retrieve version: "
    ...
    public void lock(Serializable id, Object version, Object object,
        ...
        if (lockMode != LockMode.NONE) {
            ...
            try {
                ...
                PreparedStatement st = session.getBatcher().prepareStatement(
                        getLockString(lockMode));
                try {
                    getIdentifierType().nullSafeSet(st, id, 1, session);
                    if (isVersioned()) {
                        getVersionType().nullSafeSet(st, version,
                ...
                throw new JDBCException("could not lock: "
                        + MessageHelper.infoString(this, id), sqle);
            }
    ...
    public Object[] getCurrentPersistentState(Serializable id, Object version,
            SessionImplementor session) throws HibernateException {
        ...
        try {
            PreparedStatement ps = session.getBatcher().prepareQueryStatement(
                    getConcreteSelectString(), false);
            ResultSet rs = null;
            try {
                getIdentifierType().nullSafeSet(ps, id, 1, session);
                getVersionType().nullSafeSet(ps, version,
                        getIdentifierColumnNames().length + 1, session);
                ...
                for (int i = 0; i < types.length; i++) {
                    if (includeProperty[i]) {
                        values[i] = types[i].hydrate(rs, getPropertyAliases(
                                StringHelper.EMPTY_STRING, i), session, null); //null
                        // owner
                        // ok??
                    }
                }
    ...
    protected String generateSelectVersionString() {
        ...
        if (isVersioned()) {
            select.addColumn(getVersionColumnName());
        } else {
            select.addColumns(getIdentifierColumnNames());
        }
    ...
    public Object createProxy(Serializable id, SessionImplementor session)
            throws HibernateException {
        return CGLIBLazyInitializer.getProxy(proxyFactory, mappedClass,
                proxyInterfaces, proxyGetIdentifierMethod,
                proxySetIdentifierMethod, id, session);
    }
    ...
    protected final boolean[] getPropertiesToUpdate(final int[] dirtyProperties) {
        ...
        if (isVersioned())
            propsToUpdate[getVersionProperty()] = true;
