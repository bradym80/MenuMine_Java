//$Id: NormalizedEntityPersister.java,v 1.4 2005/08/24 04:33:28 nick Exp $
...
public class NormalizedEntityPersister extends AbstractEntityPersister {
    ...
    private static final Log log = LogFactory
            .getLog(NormalizedEntityPersister.class);
    ...
    public String getDiscriminatorAlias() {
        return getDiscriminatorColumnName(); // is always "clazz_", so just use
        // columnname
    }
    ...
    public Serializable[] getPropertySpaces() {
        return tableNames; // don't need subclass tables, because they can't
            ...
            for (int i = 0; i < propertyColumnNames.length; i++) {
                if (includeProperty[i] && naturalOrderPropertyTables[i] == j) {
                    ...
                    hasColumns = hasColumns
                            || propertyColumnNames[i].length > 0;
                }
            }
    ...
    protected String generateLockString() {
        ...
        SimpleSelect select = new SimpleSelect().setTableName(
                qualifiedTableName).addColumn(
                super.getIdentifierColumnNames()[0]).addCondition(
                super.getIdentifierColumnNames(), "=?");
        if (isVersioned()) {
            select.addWhereToken("and").addCondition(getVersionColumnName(),
                    "=?");
        }
    ...
    protected String generateConcreteSelectString() {
        String select = "select "
                + StringHelper.join(StringHelper.COMMA_SPACE, StringHelper
                        .qualify(CONCRETE_ALIAS, getIdentifierColumnNames()))
                + concretePropertySelectFragment(CONCRETE_ALIAS,
                ...
                + StringHelper.join("=? and ", StringHelper.qualify(
                        CONCRETE_ALIAS, getIdentifierColumnNames())) + "=?";
        if (isVersioned()) {
            select += " and " + getVersionColumnName() + "=?";
        }
        return select;
    }
    ...
    protected int dehydrate(Serializable id, Object[] fields,
            boolean[] includeProperty, PreparedStatement[] statements,
        ...
        for (int i = 0; i < tableNames.length; i++) {
            int index = dehydrate(id, fields, includeProperty, i,
                    statements[i], session);
            if (i == 0)
                versionParam = index;
        }
    ...
    private int dehydrate(Serializable id, Object[] fields,
            boolean[] includeProperty, int table, PreparedStatement statement,
            SessionImplementor session) throws SQLException, HibernateException {
        ...
        for (int j = 0; j < getHydrateSpan(); j++) {
            ...
            if (includeProperty[j] && naturalOrderPropertyTables[j] == table) {
                getPropertyTypes()[j].nullSafeSet(statement, fields[j], index,
                        session);
     ...
     * joining <tt>loader</tt>, depending upon the value of the <tt>lock</tt>
     * parameter
     */
    public Object load(Serializable id, Object optionalObject,
        ...
        if (log.isTraceEnabled())
            log.trace("Materializing entity: "
                    + MessageHelper.infoString(this, id));
        ...
        try {
            ...
            throw new JDBCException("could not load by id: "
                    + MessageHelper.infoString(this, id), sqle);
        }
    }
    ...
    public Serializable insert(Object[] fields, Object object,
        ...
        if (useDynamicInsert()) {
            ...
            return insert(fields, notNull,
                    generateInsertStrings(true, notNull), object, session);
        } else {
            return insert(fields, getPropertyInsertability(),
                    getSQLIdentityInsertStrings(), object, session);
        }
    }
    ...
    public void insert(Serializable id, Object[] fields, Object object,
        ...
        if (useDynamicInsert()) {
            boolean[] notNull = getNotNullInsertableColumns(fields);
            insert(id, fields, notNull, generateInsertStrings(false, notNull),
                    object, session);
        } else {
            insert(id, fields, getPropertyInsertability(),
                    getSQLInsertStrings(), object, session);
        }
    }
     ...
     * Persist an object
     */
    public void insert(Serializable id, Object[] fields, boolean[] notNull,
            String[] sql, Object object, SessionImplementor session)
            throws HibernateException {
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < tableNames.length; i++) {
                    statements[i] = session.getBatcher().prepareStatement(
                            sql[i]);
                }

                // Write the values of fields onto the prepared statement - we
                // MUST use the state at the time the
                // insert was issued (cos of foreign key constraints). Not
                ...
                for (int i = 0; i < tableNames.length; i++) {
                    if (statements[i] != null)
                        session.getBatcher().closeStatement(statements[i]);
                }
            }
        } catch (SQLException sqle) {
            throw new JDBCException("could not insert: "
                    + MessageHelper.infoString(this, id), sqle);
        }
     ...
     */
    public Serializable insert(Object[] fields, boolean[] notNull,
            String[] sql, Object object, SessionImplementor session)
            throws HibernateException {
        ...
        try {
            ...
            PreparedStatement statement = session.getBatcher()
                    .prepareStatement(sql[0]);
            try {
            ...
            PreparedStatement idselect = session.getBatcher().prepareStatement(
                    sqlIdentitySelect());
            ...
            try {
                ...
                try {
                    ...
                    id = IdentifierGeneratorFactory.get(rs,
                            getIdentifierType(), session, object);
                } finally {
                    rs.close();
                }
            ...
            throw new JDBCException("could not insert: "
                    + MessageHelper.infoString(this), sqle);
        }
    ...
    public void delete(Serializable id, Object version, Object object,
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < naturalOrderTableNames.length; i++) {
                    statements[i] = session.getBatcher().prepareStatement(
                            getSQLDeleteStrings()[i]);
                }

                if (isVersioned())
                    getVersionType().nullSafeSet(statements[0], version,
                            getIdentifierColumnNames().length + 1, session);

                for (int i = naturalOrderTableNames.length - 1; i >= 0; i--) {

                    // Do the key. The key is immutable so we can use the
                    // _current_ object state - not necessarily
                    // the state at the time the delete was issued

                    getIdentifierType().nullSafeSet(statements[i], id, 1,
                            session);
            ...
            throw new JDBCException("could not delete: "
                    + MessageHelper.infoString(this, id), sqle);
        }
    }
    ...
    public void update(Serializable id, Object[] fields, int[] dirtyFields,
            Object[] oldFields, Object oldVersion, Object object,
            SessionImplementor session) throws HibernateException {
        ...
        update(id, fields, propsToUpdate, tableUpdateNeeded, oldVersion,
                object, updateStrings, session);
    }

    protected void update(Serializable id, Object[] fields,
            boolean[] includeProperty, boolean[] includeTable,
            Object oldVersion, Object object, String[] sql,
            SessionImplementor session) throws HibernateException {
        ...
        try {
            ...
            try {
                ...
                int versionParam = dehydrate(id, fields, includeProperty,
                        statements, session);

                if (isVersioned())
                    getVersionType().nullSafeSet(statements[0], oldVersion,
                            versionParam, session);
    ...
    public NormalizedEntityPersister(PersistentClass model,
        ...
        qualifiedTableName = table.getQualifiedName(getDialect(), factory
                .getDefaultSchema());
        ...
        while (titer.hasNext()) {
            ...
            String tabname = tab.getQualifiedName(getDialect(), factory
                    .getDefaultSchema());
            if (!tabname.equals(qualifiedTableName)) {
                ...
                for (int k = 0; k < idColumnSpan; k++)
                    key[k] = ((Column) kiter.next())
                            .getQuotedName(getDialect());
                keyColumns.add(key);
            }
        }
        naturalOrderTableNames = (String[]) tables
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        naturalOrderTableKeyColumns = (String[][]) keyColumns
        ...
        subclassTableNameClosure = (String[]) subtables
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassTableKeyColumns = (String[][]) keyColumns
                .toArray(new String[0][]);
        isClassOrSuperclassTable = new boolean[subclassTableNameClosure.length];
        for (int j = 0; j < subclassTableNameClosure.length; j++) {
            isClassOrSuperclassTable[j] = tables
                    .contains(subclassTableNameClosure[j]);
        }
        ...
        while (iter.hasNext()) {
            ...
            naturalOrderPropertyTables[i] = getTableId(tabname,
                    naturalOrderTableNames);

            if (prop.isFormula()) {
                propertyColumnAliases[i] = new String[] { prop.getFormula()
                        .getAlias() };
                propertyColumnSpans[i] = 1;
                propertyFormulaTemplates[i] = prop.getFormula().getTemplate(
                        getDialect());
                foundFormula = true;
            } else {
                ...
                while (colIter.hasNext()) {
                    ...
                    propCols[j] = colname;
                    propAliases[j] = col.getAlias() + tab.getUniqueInteger()
        ...
        checkColumnDuplication(distinctColumns, model.getKey()
                .getColumnIterator());
        iter = model.getPropertyIterator();
        while (iter.hasNext()) {
            Property prop = (Property) iter.next();
            if (prop.isUpdateable() || prop.isInsertable()) {
                checkColumnDuplication(distinctColumns, prop
                        .getColumnIterator());
            }
        }
        ...
        while (iter.hasNext()) {
            ...
            definedBySubclass.add(new Boolean(!thisClassProperties
                    .contains(prop)));
            Table tab = prop.getValue().getTable();
            String tabname = tab.getQualifiedName(getDialect(), factory
                    .getDefaultSchema());
            Integer tabnum = new Integer(getTableId(tabname,
                    subclassTableNameClosure));
            propTables.add(tabnum);
            types.add(prop.getType());

            if (prop.isFormula()) {
                formulaTemplates.add(prop.getFormula()
                        .getTemplate(getDialect()));
                ...
                while (colIter.hasNext()) {
                    ...
                    aliases.add(col.getAlias() + tab.getUniqueInteger()
                            + StringHelper.UNDERSCORE);
                }
                propColumns.add(cols);
            }
            joinedFetchesList.add(new Integer(prop.getValue()
                    .getOuterJoinFetchSetting()));
        }
        subclassColumnClosure = (String[]) columns
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassColumnClosureAliases = (String[]) aliases
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassColumnTableNumberClosure = ArrayHelper.toIntArray(coltables);
        subclassPropertyTypeClosure = (Type[]) types
                .toArray(ArrayHelper.EMPTY_TYPE_ARRAY);
        subclassPropertyNameClosure = (String[]) names
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassPropertyTableNumberClosure = ArrayHelper.toIntArray(propTables);
        subclassFormulaAliasClosure = (String[]) formulaAliases
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassFormulaTemplateClosure = (String[]) formulaTemplates
                .toArray(ArrayHelper.EMPTY_STRING_ARRAY);
        subclassFormulaTableNumberClosure = ArrayHelper.toIntArray(formtables);
        subclassPropertyColumnNameClosure = (String[][]) propColumns
                .toArray(new String[propColumns.size()][]);

        subclassPropertyEnableJoinedFetch = new int[joinedFetchesList.size()];
        iter = joinedFetchesList.iterator();
        int j = 0;
        while (iter.hasNext())
            subclassPropertyEnableJoinedFetch[j++] = ((Integer) iter.next())
                    .intValue();
        ...
        sqlInsertStrings = generateInsertStrings(false,
                getPropertyInsertability());
        sqlIdentityInsertStrings = isIdentifierAssignedByInsert() ? generateInsertStrings(
        ...
        if (model.isPolymorphic()) {
            ...
            int id = getTableId(model.getTable().getQualifiedName(getDialect(),
                    factory.getDefaultSchema()), subclassTableNameClosure);
            tableNumbers[subclassSpan - 1] = id;
            notNullColumns = new String[subclassSpan];
            notNullColumns[subclassSpan - 1] = subclassTableKeyColumns[id][0]; //(
            // (Column)
            // model.getTable().getPrimaryKey().getColumnIterator().next()
            // ).getName();
        } else {
        ...
        while (iter.hasNext()) {
            ...
            try {
                if (model.isPolymorphic()) {
                    ...
                    subclassesByDiscriminatorValue.put(disc, sc
                            .getMappedClass());
                    discriminatorValues[k] = disc.toString();
                    int id = getTableId(sc.getTable().getQualifiedName(
                            getDialect(), factory.getDefaultSchema()),
                            subclassTableNameClosure);
                    tableNumbers[k] = id;
                    notNullColumns[k] = subclassTableKeyColumns[id][0]; //(
                    // (Column)
    ...
    protected void handlePath(String path, Type type) {
        if (type.isAssociationType()
            ...
            tableNumberByPropertyPath.put(path, new Integer(
                    getPropertyTableNumber(propertyName)));
        }
    }
    ...
    private JoinFragment outerjoin(String name, boolean innerJoin,
            ...
            if (includeSubclasses || isClassOrSuperclassTable[i]) {
                outerjoin
                        .addJoin(
                                ...
                                alias(name, i),
                                StringHelper.qualify(name,
                                        getIdentifierColumnNames()),
                                subclassTableKeyColumns[i],
                                innerJoin && isClassOrSuperclassTable[i] ? JoinFragment.INNER_JOIN
    ...
    private static int getTableId(String tableName, String[] tables) {
        for (int tab = 0; tab < tables.length; tab++) {
            if (tableName.equals(tables[tab]))
                return tab;
        }
        throw new AssertionFailure("table not found");
    }

    public String[] toColumns(String alias, String property)
        ...
        int tab = ((Integer) tableNumberByPropertyPath.get(property))
                .intValue();
    ...
    public String[] toColumns(String alias, int i) {
        ...
        return StringHelper.qualify(alias(alias, tab),
                subclassPropertyColumnNameClosure[i]);
    }

    private String concretePropertySelectFragment(String alias,
            boolean[] includeProperty) {
        int propertyCount = getPropertyNames().length;
        SelectFragment frag = new SelectFragment();
        for (int i = 0; i < propertyCount; i++) {
            if (includeProperty[i]) { //ie. updateable, not a formula
                frag.addColumns(alias(alias, propertyTables[i]),
                        propertyColumnNames[i], propertyColumnAliases[i]);
    ...
    public String propertySelectFragment(String alias, String suffix) {
        ...
        for (int i = 0; i < subclassColumnClosure.length; i++) {
            ...
            frag.addColumn(subalias, subclassColumnClosure[i],
        ...
        for (int i = 0; i < subclassFormulaTemplateClosure.length; i++) {
            ...
            frag.addFormula(subalias, subclassFormulaTemplateClosure[i],
                    subclassFormulaAliasClosure[i]);
        }

        if (hasSubclasses()) {
            return ", "
                    + discriminatorFragment(alias).setReturnColumnName(
                            getDiscriminatorAlias(), suffix).toFragmentString()
    ...
    private CaseFragment discriminatorFragment(String alias) {
        ...
        for (int i = 0; i < discriminatorValues.length; i++) {
            cases.addWhenColumnNotNull(alias(alias, tableNumbers[i]),
                    notNullColumns[i], discriminatorValues[i]);
        }
    ...
    public String fromJoinFragment(String alias, boolean innerJoin,
            boolean includeSubclasses) {
        return outerjoin(alias, innerJoin, includeSubclasses)
    ...
    public String queryWhereFragment(String alias, boolean innerJoin,
            boolean includeSubclasses) throws MappingException {
        String result = whereJoinFragment(alias, innerJoin, includeSubclasses);
        String rootAlias = alias(alias, naturalOrderTableNames.length - 1); //urgh,
        // ugly!
        if (hasWhere())
            result += " and " + getSQLWhereString(rootAlias);
        return result;
    }
