//$Id: SimpleExpression.java,v 1.2 2005/04/03 06:29:03 nick Exp $
...
public abstract class SimpleExpression extends AbstractCriterion {
    ...
    public String toSqlString(SessionFactoryImplementor sessionFactory,
        ...
        String[] columns = getColumns(sessionFactory, persistentClass,
                propertyName, alias, aliasClasses);
        if (ignoreCase) {
            if (columns.length != 1)
            ...
            return new StringBuffer().append(
                    sessionFactory.getDialect().getLowercaseFunction()).append(
                    StringHelper.OPEN_PAREN).append(columns[0]).append(
                    StringHelper.CLOSE_PAREN).append(getOp()).append("?")
                    .toString();
        } else {
            String result = StringHelper.join(" and ", StringHelper.suffix(
                    columns, getOp() + "?"));
            if (columns.length > 1)
                result = StringHelper.OPEN_PAREN + result
                        + StringHelper.CLOSE_PAREN;
            return result;
        }
    ...
    public TypedValue[] getTypedValues(
        ...
        return new TypedValue[] { getTypedValue(sessionFactory,
                persistentClass, propertyName, icvalue, aliasClasses) };
    }
