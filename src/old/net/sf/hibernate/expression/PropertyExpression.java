//$Id: PropertyExpression.java,v 1.2 2005/04/03 06:29:03 nick Exp $
...
public abstract class PropertyExpression extends AbstractCriterion {
    ...
    public String toSqlString(SessionFactoryImplementor sessionFactory,
        ...
        String[] xcols = getColumns(sessionFactory, persistentClass,
                propertyName, alias, aliasClasses);
        String[] ycols = getColumns(sessionFactory, persistentClass,
                otherPropertyName, alias, aliasClasses);
        String result = StringHelper.join(" and ", StringHelper.add(xcols,
                getOp(), ycols));
        if (xcols.length > 1)
