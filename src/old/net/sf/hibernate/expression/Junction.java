...
public abstract class Junction extends AbstractCriterion {
    ...
    public TypedValue[] getTypedValues(
        ...
        while (iter.hasNext()) {
            TypedValue[] subvalues = ((Criterion) iter.next()).getTypedValues(
                    sessionFactory, persistentClass, aliasClasses);
            for (int i = 0; i < subvalues.length; i++) {
                typedValues.add(subvalues[i]);
            }
        }
        return (TypedValue[]) typedValues.toArray(new TypedValue[typedValues
                .size()]);
    }
    ...
    public String toSqlString(SessionFactoryImplementor sessionFactory,
        ...
        Iterator iter = criteria.iterator();
        while (iter.hasNext()) {
            buffer.append(((Criterion) iter.next()).toSqlString(sessionFactory,
