//$Id: Example.java,v 1.2 2005/04/03 06:29:03 nick Exp $
 ...
 * 
 * <pre>
 * List results = session.createCriteria(Parent.class).add(
 *         Example.create(parent).ignoreCase()).createCriteria(&quot;child&quot;).add(
 *         Example.create(parent.getChild())).list();
 * </pre>
 * 
...
public class Example extends AbstractCriterion {
            ...
            if (isPropertyIncluded) {
                if (propertyTypes[i].isComponentType()) {
                    addComponentTypedValues(name, value,
                            (AbstractComponentType) type, list);
                } else {
    ...
    protected void addComponentTypedValues(String path, Object component,
            AbstractComponentType type, List list) throws HibernateException {
    ...
    protected void appendPropertyCondition(String propertyName,
            Object propertyValue, Class persistentClass, String alias,
            Map aliasClasses, SessionFactoryImplementor sessionFactory,
        ...
        if (propertyValue != null) {
            ...
            crit = (isLikeEnabled && isString) ? (Criterion) new LikeExpression(
                    propertyName, propertyValue, isIgnoreCaseEnabled)
                    : (Criterion) new EqExpression(propertyName, propertyValue,
                            isIgnoreCaseEnabled && isString);

        } else {
            crit = new NullExpression(propertyName);
        }
        buf.append(crit.toSqlString(sessionFactory, persistentClass, alias,
                aliasClasses));
    }

    protected void appendComponentCondition(String path, Object component,
            AbstractComponentType type, Class persistentClass, String alias,
            Map aliasClasses, SessionFactoryImplementor sessionFactory,
            StringBuffer buf) throws HibernateException {
        ...
        if (component != null) {
            ...
            for (int i = 0; i < propertyNames.length; i++) {
                ...
                if (isPropertyIncluded(value, subpath, subtypes[i])) {
                    appendPropertyCondition(subpath, value, persistentClass,
                            alias, aliasClasses, sessionFactory, buf);
                }
            }
        }
    }
