//$Id: C3P0ConnectionProvider.java,v 1.3 2005/04/03 06:29:04 nick Exp $
...
public class C3P0ConnectionProvider implements ConnectionProvider {
    ...
    private static final Log log = LogFactory
            .getLog(C3P0ConnectionProvider.class);
    ...
    public void configure(Properties props) throws HibernateException {
        ...
        try {
            ...
            int minPoolSize = PropertiesHelper.getInt(
                    Environment.C3P0_MIN_SIZE, props, 1);
            int maxPoolSize = PropertiesHelper.getInt(
                    Environment.C3P0_MAX_SIZE, props, 100);
            int maxIdleTime = PropertiesHelper.getInt(Environment.C3P0_TIMEOUT,
                    props, 0);
            int maxStatements = PropertiesHelper.getInt(
                    Environment.C3P0_MAX_STATEMENTS, props, 0);
            boolean validateConnection = PropertiesHelper.getBoolean(
                    Environment.C3P0_VALIDATE_CONNECTION, props);

            PoolConfig pcfg = new PoolConfig();
            pcfg.setMinPoolSize(minPoolSize);
            pcfg.setMaxPoolSize(maxPoolSize);
            pcfg.setAcquireIncrement(1); //hardcoded value, as before
