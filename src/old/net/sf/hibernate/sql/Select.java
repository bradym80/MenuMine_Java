//$Id: Select.java,v 1.2 2005/04/03 06:29:05 nick Exp $
...
public class Select {
    ...
    public String toStatementString() {
        StringBuffer buf = new StringBuffer(selectClause.length()
                + fromClause.length() + outerJoinsAfterFrom.length()
                + whereClause.length() + outerJoinsAfterWhere.length() + 20);
        buf.append("select ").append(selectClause).append(" from ").append(
                fromClause).append(outerJoinsAfterFrom).append(" where ")
                .append(whereClause).append(outerJoinsAfterWhere);
     ...
     * Sets the outerJoins.
     * 
     * @param outerJoinsAfterFrom
     *            The outerJoinsAfterFrom to set
     * @param outerJoinsAfterWhere
