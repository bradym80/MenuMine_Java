//$Id: QueryJoinFragment.java,v 1.2 2005/04/03 06:29:05 nick Exp $
...
public class QueryJoinFragment extends JoinFragment {
    ...
    public void addJoin(String tableName, String alias, String[] fkColumns,
            String[] pkColumns, int joinType) {
        addJoin(tableName, alias, alias, fkColumns, pkColumns, joinType);
    }

    private void addJoin(String tableName, String alias, String concreteAlias,
            String[] fkColumns, String[] pkColumns, int joinType) {
    ...
    public void addCondition(String alias, String[] columns, String condition) {
        for (int i = 0; i < columns.length; i++) {
            afterWhere.append(" and ").append(alias).append(StringHelper.DOT)
                    .append(columns[i]).append(condition);
        }
    }

    public void addCrossJoin(String tableName, String alias) {
        afterFrom.append(StringHelper.COMMA_SPACE).append(tableName)
                .append(' ').append(alias);
    }
    ...
    public void addCondition(String alias, String[] fkColumns,
        ...
        for (int j = 0; j < fkColumns.length; j++) {
            afterWhere.append(" and ").append(fkColumns[j]).append('=').append(
                    alias).append(StringHelper.DOT).append(pkColumns[j]);
        }
    }
