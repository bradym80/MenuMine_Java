//$Id: ComponentType.java,v 1.3 2005/08/24 04:33:25 nick Exp $
...
public class ComponentType extends AbstractType implements
    ...
    public ComponentType(Class componentClass, String[] propertyNames,
            Getter[] propertyGetters, Setter[] propertySetters,
            final boolean foundCustomAccessor, Type[] propertyTypes,
            int[] joinedFetch, Cascades.CascadeStyle[] cascade,
            String parentProperty) throws MappingException {
        ...
        if (parentProperty == null) {
            ...
            PropertyAccessor pa = PropertyAccessorFactory
                    .getPropertyAccessor(null);
        ...
        optimizer = !foundCustomAccessor
                ...
                .getBulkBean(componentClass, getterNames, setterNames,
                        propTypes) : null;
    ...
    public boolean isDirty(Object x, Object y, SessionImplementor session)
        ...
        for (int i = 0; i < getters.length; i++) {
            if (propertyTypes[i].isDirty(getters[i].get(x), getters[i].get(y),
                    session))
                return true;
        }
        return false;
        ...
        if (optimizer != null) {
            try {
                ...
                throw new PropertyAccessException(t,
                        ...
                        componentClass, ReflectHelper.getPropertyName(t,
                                optimizer));
            }
        } else {
    ...
    public void setPropertyValues(Object component, Object[] values)
        ...
        if (optimizer != null) {
            try {
                ...
                throw new PropertyAccessException(t,
                        ReflectHelper.PROPERTY_ACCESS_EXCEPTION, true,
                        componentClass, ReflectHelper.getPropertyName(t,
                                optimizer));
            }
        } else {
    ...
    public String toString(Object value, SessionFactoryImplementor factory)
        ...
        for (int i = 0; i < propertyTypes.length; i++) {
            result.put(propertyNames[i], propertyTypes[i].toString(values[i],
                    factory));
        }
        ...
        if (optimizer != null) {
            try {
                ...
                throw new InstantiationException(
                        "Could not instantiate component with CGLIB: ",
                        componentClass, t);
            }
        } else {
            try {
                ...
                throw new InstantiationException(
                        "Could not instantiate component: ", componentClass, e);
            }
        }
    }
    ...
    public Object hydrate(ResultSet rs, String[] names,
        ...
        for (int i = 0; i < propertySpan; i++) {
            ...
            String[] range = ArrayHelper.slice(names, begin, length); //cache
            // this
    ...
    public Object resolveIdentifier(Object value, SessionImplementor session,
        ...
        if (value != null) {
            ...
            for (int i = 0; i < values.length; i++)
                values[i] = propertyTypes[i].resolveIdentifier(values[i],
                        session, owner);
            setPropertyValues(result, values);
            return result;
        } else {
    ...
    public boolean isModified(Object old, Object current,
        ...
        for (int i = 0; i < currentValues.length; i++) {
            if (propertyTypes[i].isModified(oldValues[i], currentValues[i],
                    session))
                return true;
        }
        return false;
