//$Id: NullableType.java,v 1.3 2005/04/03 06:29:03 nick Exp $
...
public abstract class NullableType extends AbstractType {
    ...
    static {
        ...
        IS_TRACE_ENABLED = LogFactory.getLog(
    ...
    public final void nullSafeSet(PreparedStatement st, Object value,
        ...
        if (value == null) {
            if (IS_TRACE_ENABLED)
                LogFactory.getLog(getClass()).trace(
                        "binding null to parameter: " + index);

            st.setNull(index, sqlType());
        } else {
            if (IS_TRACE_ENABLED)
                LogFactory.getLog(getClass()).trace(
                        "binding '" + toString(value) + "' to parameter: "
                                + index);
    ...
    public final Object nullSafeGet(ResultSet rs, String name)
        ...
        if (value == null || rs.wasNull()) {
            ...
            if (IS_TRACE_ENABLED)
                LogFactory.getLog(getClass()).trace(
                        "returning '" + toString(value) + "' as column: "
                                + name);
            return value;
        }
    }
    ...
    public final String toString(Object value, SessionFactoryImplementor pc)
            throws HibernateException {
