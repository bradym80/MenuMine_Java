//$Id: DynamicComponentType.java,v 1.4 2005/08/24 04:33:25 nick Exp $
 ...
 * @author Gavin King
 */
public class DynamicComponentType extends AbstractType implements
    ...
    public DynamicComponentType(String[] propertyNames, Type[] propertyTypes,
    ...
    public boolean equals(Object x, Object y) throws HibernateException {
        ...
        Map ybean = (Map) y;
        for (int i = 0; i < propertySpan; i++) {
            if (!propertyTypes[i].equals(xbean.get(propertyNames[i]), ybean
                    .get(propertyNames[i])))
                return false;
        }
        return true;
    ...
    public boolean isDirty(Object x, Object y, SessionImplementor session)
        ...
        Map ybean = (Map) y;
        for (int i = 0; i < propertySpan; i++) {
            if (propertyTypes[i].isDirty(xbean.get(propertyNames[i]), ybean
                    .get(propertyNames[i]), session))
                return true;
        }
        return false;
    }
    ...
    public Object nullSafeGet(ResultSet rs, String[] names,
        ...
        for (int i = 0; i < propertySpan; i++) {
            ...
            // this
            Object val = propertyTypes[i]
                    .nullSafeGet(rs, range, session, owner);
            if (val != null)
                notNull = true;
            values[i] = val;
            begin += length;
