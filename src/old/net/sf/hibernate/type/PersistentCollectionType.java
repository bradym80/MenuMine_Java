//$Id: PersistentCollectionType.java,v 1.2 2005/04/03 06:29:03 nick Exp $
...
/**
 * A type that handles Hibernate <tt>PersistentCollection</tt> s (including
 * arrays).
 * 
 * @author Gavin King
 */
public abstract class PersistentCollectionType extends AbstractType implements
        AssociationType {
    ...
    public abstract PersistentCollection instantiate(
    ...
    public Object nullSafeGet(ResultSet rs, String[] name,
            SessionImplementor session, Object owner)
            throws HibernateException, SQLException {
        return resolveIdentifier(hydrate(rs, name, session, owner), session,
                owner);
    }
    ...
    public Serializable disassemble(Object value, SessionImplementor session)
        ...
        return null;
        /*
         * if (value==null) { return null; } else { Serializable id =
         * session.getLoadedCollectionKey( (PersistentCollection) value ); if
         * (id==null) throw new AssertionFailure("Null collection id"); return
         * id; }
         */
    }
    ...
    private boolean isOwnerVersioned(SessionImplementor session)
            throws MappingException {
        Class ownerClass = session.getFactory().getCollectionPersister(role)
                .getOwnerClass();
    ...
    public boolean isDirty(Object old, Object current,
        ...
        return isOwnerVersioned(session)
                && super.isDirty(old, current, session);
        //return false;
     ...
     * <tt>AssociationType</tt>. Not because all collections are
     * associations.
     */
    ...
    public Object hydrate(ResultSet rs, String[] name,
            SessionImplementor session, Object owner)
            throws HibernateException, SQLException {
    ...
    public Object resolveIdentifier(Object value, SessionImplementor session,
        ...
        if (value == null) {
            ...
            return session.getCollection(role, (Serializable) value, owner);
        }
    }
    ...
    public abstract PersistentCollection assembleCachedCollection(
            SessionImplementor session, CollectionPersister persister,
    ...
    public Class getAssociatedClass(SessionFactoryImplementor factory)
        ...
        try {
            ...
            throw new MappingException("collection role is not queryable "
                    + role);
        }
    }
