//$Id: SessionFactoryImpl.java,v 1.4 2005/08/24 04:33:25 nick Exp $
 ...
 * extremely sparingly.
 * 
 * @see net.sf.hibernate.ps.PreparedStatementCache
...
public final class SessionFactoryImpl implements SessionFactory,
    ...
    public SessionFactoryImpl(Configuration cfg, Settings settings)
        ...
        while (classes.hasNext()) {
            ...
            ClassPersister cp = PersisterFactory.createClassPersister(model,
                    this);
        ...
        while (collections.hasNext()) {
            ...
            CollectionPersister persister = PersisterFactory
                    .createCollectionPersister(cfg, map, this);
        ...
        while (namedIter.hasNext()) {
            ...
            namedSqlQueries.put(entry.getKey(), new InternalNamedSQLQuery(nsq
                    .getQueryString(), nsq.getReturnAliases(), nsq
                    .getReturnClasses(), nsq.getSynchronizedTables()));
        }
        ...
        if (settings.isQueryCacheEnabled()) {
            ...
            queryCache = new QueryCache(settings.getCacheProvider(),
                    properties, updateTimestampsCache, null);
    ...
    private final transient Object[] strongRefs = new Object[MAX_STRONG_REF_COUNT]; //strong
    ...
    private final transient Map softQueryCache = new ReferenceMap(
            ReferenceMap.SOFT, ReferenceMap.SOFT);

    // both keys and values may be soft since value keeps a hard ref to the key
    // (and there is a hard ref to MRU values)
    ...
    static {
        QUERY_KEY_FACTORY = (QueryCacheKeyFactory) KeyFactory
                .create(QueryCacheKeyFactory.class);
        FILTER_KEY_FACTORY = (FilterCacheKeyFactory) KeyFactory
                .create(FilterCacheKeyFactory.class);
    }
    ...
    //TODO: this stuff can be implemented in separate class to reuse soft
    // MRU/LRU caching
    ...
    private synchronized QueryTranslator[] createQueryTranslators(
            String[] concreteQueryStrings, Object cacheKey) {
    ...
    private synchronized FilterTranslator createFilterTranslator(
            String filterString, Object cacheKey) {
        final FilterTranslator filter = new FilterTranslator(filterString);
        put(cacheKey, filter);
        return filter;
    }

    public QueryTranslator[] getQuery(String queryString, boolean shallow)
            throws QueryException, MappingException {

        Object cacheKey = QUERY_KEY_FACTORY.newInstance(queryString, shallow);

        // have to be careful to ensure that if the JVM does out-of-order
        // execution
        // then another thread can't get an uncompiled QueryTranslator from the
        // cache
        // we also have to be very careful to ensure that other threads can
        // perform
        // compiled queries while another query is being compiled

        QueryTranslator[] queries = (QueryTranslator[]) get(cacheKey);
        if (queries == null) {
            // a query that names an interface or unmapped class in the from
            // clause
            // is actually executed as multiple queries
            String[] concreteQueryStrings = QueryTranslator.concreteQueries(
                    queryString, this);
        ...
        // see comment above. note that QueryTranslator.compile() is
        // synchronized
        return queries;
    ...
    public FilterTranslator getFilter(String filterString,
            String collectionRole, boolean scalar) throws QueryException,
            MappingException {

        Object cacheKey = FILTER_KEY_FACTORY.newInstance(collectionRole,
                filterString, scalar);

        FilterTranslator filter = (FilterTranslator) get(cacheKey);
        if (filter == null)
            filter = createFilterTranslator(filterString, cacheKey);
        filter.compile(collectionRole, this, settings.getQuerySubstitutions(),
                scalar);
        // see comment above. note that FilterTranslator.compile() is
        // synchronized
        return filter;

    }

    private Session openSession(Connection connection, boolean autoClose,
            long timestamp, Interceptor interceptor) {
    ...
    public ClassPersister getPersister(String className)
        ...
        ClassPersister result = (ClassPersister) classPersistersByName
                .get(className);
        if (result == null)
    ...
    public CollectionPersister getCollectionPersister(String role)
        ...
        CollectionPersister result = (CollectionPersister) collectionPersisters
                .get(role);
        if (result == null)
            throw new MappingException("No persister for collection role: "
                    + role);
        return result;
    }
    ...
    public Type[] getReturnTypes(String queryString) throws HibernateException {
        ...
        if (queries.length == 0)
            throw new HibernateException(
                    "Query does not refer to any persistent classes: "
                            + queryString);
                ...
                if (q.isExplicitPolymorphism()) {
                    ...
                    if (isMappedClass) {
                        results.add(className);
                            ...
                            && (!q.isInherited() || !clazz.isAssignableFrom(q
                                    .getMappedSuperclass()))) {
                        results.add(className);
                    }
                }
    ...
    public QueryCache getQueryCache(String cacheRegion)
        ...
        if (currentQueryCache == null) {
            currentQueryCache = new QueryCache(settings.getCacheProvider(),
                    properties, updateTimestampsCache, cacheRegion);
    ...
    static class InternalNamedSQLQuery {
        ...
        public InternalNamedSQLQuery(String query, String[] aliases,
                Class[] clazz, List querySpaces) {
