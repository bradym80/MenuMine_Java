//$Id: Database.java,v 1.3 2005/04/03 06:29:05 nick Exp $
 ...
 * persisting and deleteing objects, binding names to objects and looking up
 * objects by name. <br>
 * <br>
 * Applications may create an instance by calling
 * <tt>Implementation.newDatabase()</tt> or by instantiating this class
 * directly. The application should then call <tt>open()</tt> to associate it
 * with an existing Hibernate <tt>SessionFactory</tt>.<br>
 * <br>
 * Instances maintain an association between <tt>Thread</tt> s and</tt>
 * Transaction</tt>s.
 * 
 * @see Implementation
 * @see Transaction
...
public class Database implements org.odmg.Database {
    ...
    /**
     * Get the <tt>Session</tt> underlying the <tt>Transaction</tt>
    ...
    /**
     * Specify the underlying <tt>SessionFactory</tt>, by passing a JNDI
     * name. The <tt>accessMode</tt> is ignored by Hibernate.
     * 
    ...
    public void open(String name, int accessMode) throws ODMGException {
        ...
        sessionFactory = (SessionFactory) SessionFactoryObjectFactory
                .getNamedInstance(name);
        if (sessionFactory == null)
            throw new ODMGException(
                    "No SessionFactory was associated with the given JDNI name");
        /*
         * try { sessionFactory = (SessionFactory)
         * NamingHelper.getInitialContext( Environment.getProperties()
         * ).lookup(name); } catch (NamingException ne) { throw new
         * ODMGException( ne.getMessage() ); }
         */
    }

    /**
     * Close the <tt>Database</tt> (but not the underlying
    ...
    public void close() throws ODMGException {
        ...
        //if ( !threadTransactions.isEmpty() ) throw new ODMGException("Running
        // transactions");
        sessionFactory = null;
    ...
    /**
     * Bind a name to a persistent object, making the object persistent if
     * necessary.
     * 
