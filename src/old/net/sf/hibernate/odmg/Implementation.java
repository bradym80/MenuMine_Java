//$Id: Implementation.java,v 1.3 2005/04/03 06:29:05 nick Exp $
...
public class Implementation implements org.odmg.Implementation {
    ...
    /**
     * Get the <tt>Database</tt> associated with the current thread (the most
     * recent database created by the current thread or the default database).
     */
    ...
    /**
     * Create a new <tt>Database</tt> and associate it with the current
     * thread. The first <tt>Database</tt> created by this method becomes the
     * "default" database that all threads that have not created databases are
     * associated with.
     * 
    ...
    /**
     * Instantiate an <tt>OQLQuery</tt> for the <tt>Database</tt> associated
    ...
    public DList newDList() {
        return new List((SessionImplementor) currentDatabase().getSession(),
                new ArrayList());
    }
     ...
     */
    public DBag newDBag() {
        return new Bag((SessionImplementor) currentDatabase().getSession(),
                new ArrayList());
    }
     ...
     */
    public DSet newDSet() {
        return new Set((SessionImplementor) currentDatabase().getSession(),
                new HashSet());
    }
     ...
     */
    public DMap newDMap() {
        return new Map((SessionImplementor) currentDatabase().getSession(),
                new HashMap());
    }
